...
for %%i in ("%~dp0..") do set "JPX_HOME=%%~fi"
...
if not defined JAVA (
    where java >nul 2>&1
    if errorlevel 1 (
        echo jpx: no Java runtime found - set JAVA_HOME or add 'java' to PATH ^(Java 25 or newer required^) 1>&2
        exit /b 1
    )
    set "JAVA=java"
)
...
if not defined JAVA_VERSION (
    echo jpx: failed to determine Java version from '!JAVA! -version' 1>&2
    exit /b 1
)
...
if errorlevel 1 (
    echo jpx: Java 25 or newer required, but '!JAVA!' reports version '!JAVA_VERSION!' 1>&2
    exit /b 1
)
