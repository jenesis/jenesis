...
if "!STAMP!"=="!VERSION!" (
    ...
    if not defined TARGET_HOME (
        where scoop >nul 2>&1
        if not errorlevel 1 (
            echo jenesis-unsafe: build/jenesis records !STAMP!, installing that version via Scoop 1>&2
...
:refuse
echo jenesis-unsafe: !REASON! 1>&2
echo jenesis-unsafe: refusing to run - this launcher only executes a released Jenesis whose published 1>&2
