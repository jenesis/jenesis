...
application image end to end in [`demo-40-native-image`](demo/demo-40-native-image). Committing the metadata under
...
- The **inferred code-quality chains** bring the same zero-config activation to static-analysis and formatting
  ...
  [`pitest`](demo/demo-24-pitest/README.md) demo shows it killing both mutants of a covered method. The detekt runner targets the
...
off. The [`test-selection`](demo/demo-23-test-selection/README.md) demo shows it editing one class and re-running only
that class's test.
...
Maven repository as `<artifact>-<version>-cyclonedx.<ext>` for publication - see the [`sbom`](demo/demo-11-sbom/README.md) demo.
...
compiler it was declared for. `demo/demo-09-annotations` runs a Java annotation processor (Immutables) and
`demo/demo-17-kotlin-plugin` a Kotlin compiler plugin (kotlinx.serialization) this way - each showing that the jar
