...
- A **bill of materials** shares a curated set of version and checksum pins across modules and projects in the
  ...
  `[<group>/]bom-<name>.properties` instead reads that file from the project's BOM locations: the `boms`
  ...
  shared file serves every module. The `bom-` prefix keeps a file reference structurally distinct from a module coordinate, since a dash
  can never occur in a Java module name (the same fact the classifier fusion relies on). The locations are
  searched in order with the first hit winning, and they are deliberately not profile-resolved, so no profile
  can swap a BOM and a build cannot come to depend on dynamic configuration. Every `bom-*.properties` file found
...
A few locations sit deliberately outside this search: `[<group>/]bom-<name>.properties` BOM files resolve from
