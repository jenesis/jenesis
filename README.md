...
- A **bill of materials** shares a curated set of version and checksum pins across modules and projects in the
  ...
  its entries merge into) that the dependencies step fetches from the module repository as
  `<module>/<version>/<module>.properties` - or, with no version, as the floating `<module>/<module>.properties`
  latest, which is only as reproducible as an unpinned module dependency and cannot carry a hash. A token of
  ...
  `strict` requires a checksum on the BOM reference itself (the fetched file is content-verified before its
  entries are trusted) and accepts BOM-provided hashes as pins for the closure check, `versions` keeps the
  pinned versions but skips all checksum verification, and `ignore` floats everything except classifier
  qualifiers and the managed version of a dependency whose declaration carries no version of its own (there the
  management is the only version source, so dropping it could only break resolution, never float it - the
  test engine's `junit-platform-console`, versioned solely through `dependencyManagement`, is the archetype). Manifests writes the declarations into a per-module `boms.properties`
  ...
  checksum, when checksums are pinned - is already supplied by a BOM entry gets no `@jenesis.pin` line, and an
  existing unguarded line for such a covered coordinate is removed as redundant (a pin that genuinely overrides
  the BOM forces a different resolution, so it is never "covered" and is refreshed as usual); the BOM itself is
  pinned instead: every versioned `@jenesis.bom` reference has its content hash written back onto the
  declaration (guard matching selects the line to refresh exactly as for pins), while floating references stay
  floating and local files need no pinning. Under `flatten` the unguarded `@jenesis.bom` declarations are
  removed and the module's resolved closure is pinned in full - the inverse migration - with platform-guarded
