...
| `jenesis.compliance.license` | system property | Read by `InferredComplianceModule` (wired by `InferredMultiProjectAssembler` as the per-module `compliance` module). License compliance is enabled by the presence of a `licensing.properties` file in the configuration directory, which drives a `LicenseCheck` over the resolved dependency licenses: `allowed` / `denied` are comma-separated lists of license names or category keywords (e.g. `permissive`), `unknown=ignore\|warn\|fail` (default `fail`) sets the policy for a dependency whose license cannot be determined, and `override.<coordinate>=<license>` records a license for a dependency that declares none. An unrecognized key fails the build. This property is a default-`true` **suppress** override: set it to `false` to keep the file in place but skip the license check. The `.license(boolean)` wither sets it directly in code. Default `true`. |
| `jenesis.compliance.vulnerability` | system property | Read by `InferredComplianceModule`, the vulnerability counterpart. Enabled by the presence of a `vulnerability.properties` file, which wires an `OsvDownload` step (querying the OSV database; `osv.endpoint=<uri>` overrides the API endpoint) feeding a `VulnerabilityCheck` over the resolved dependencies: `severity=low\|medium\|high\|critical` sets the threshold at or above which a known advisory fails the build, and `warn=true` (default `false`) reports findings without failing. An unrecognized key fails the build. This property is a default-`true` **suppress** override; the `.vulnerability(boolean)` wither sets it in code. Default `true`. |
