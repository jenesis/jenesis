...
- A **bill of materials** shares a curated set of version and checksum pins across modules and projects in the
  ...
  BOMs pin the same coordinate the first declared wins. Under the pinning modes a BOM behaves like its pins:
  ...
  every pom walked during Maven resolution, and reproducibility comes from the pin step materializing its
  entries instead - `versions` keeps the pinned versions but skips all checksum verification, and `ignore`
  ...
  expansion depends on - so every version the flattened BOM declares is materialized as a `@jenesis.pin` line
  (a coordinate in the resolved closure with its usual artifact hash, every other entry version-only), the
  build no longer depends on the BOM's unverifiable content, and the declaration itself is refreshed with the
  resolved version alone, no hash - it remains the durable upgrade anchor that a repin under `ignore` floats
  to the latest release and re-materializes. Under `flatten` the unguarded `@jenesis.bom` declarations are
  removed and the module's resolved closure is pinned in full - the inverse migration, which also drops a
  Maven BOM's non-closure entries as meaningless without their declaration - with platform-guarded
...
are not read, with reproducibility coming from the pin step materializing the imported versions as pins. The `<dependencyManagement>` block is the single source
