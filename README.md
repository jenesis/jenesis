...
back cleanly. The `MODULAR` layout wires the `module` prefix as `JenesisModuleRepository.of(JenesisRepository.Scope.MODULE)` (the
...
overlay transparently. The `MODULAR_TO_MAVEN` layout uses `JenesisModuleRepository.of(JenesisRepository.Scope.ARTIFACT)` (the public
...
`JenesisModuleRepository.of(JenesisRepository.Scope)` instead defaults to the public overlay at
