...
- A **bill of materials** shares a curated set of version and checksum pins across modules and projects in the
  ...
  defaults to the `configuration` locations - by default the project root - so one shared file serves every
  module. The `bom-` prefix keeps a file reference structurally distinct from a module coordinate, since a dash
