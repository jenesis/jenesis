...
compiles the image in one pass:
    ...
    java -Djenesis.java.native=true build/jenesis/Project.java stage
...
| `jenesis.project.configuration` | system property | Project-wide directory (relative to the project root) the inferred quality and formatting tools search for their configuration files (`checkstyle.xml`, `pmd.xml`, `spotbugs-exclude.xml`, `.editorconfig`, `.scalafmt.conf`, and so on); defaults to the project root. Each module is also searched first in its own per-module folder (`build.jenesis/` next to its `pom.xml` in the MAVEN layout, `META-INF/build.jenesis/` under its sources in the modular layouts), so per-module configuration overrides this project-wide one. Set to an empty string to drop the project-wide directory and rely only on per-module configuration. Read by the `new Project()` constructor; the searched folders are made absolute and de-duplicated, and any folder that does not exist is ignored. |
