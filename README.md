...
| `docker/`                  | `Docker.DOCKER`                  | Conventional output location for the container build context produced by the `Docker` step: a `Dockerfile` next to the `classpath/` and `modulepath/` folders it copies into the image. Produced only for modules that declare a main class. Nothing in the build runs a container tool - the folder is the input a later `docker build` (or `podman`, or a remote builder) consumes. When the assembler wires it (via a `docker.properties` in the configuration location), `Inventory` records it as `prefix.docker` and the `STAGE` module's `docker` step (`ImageStaging`) collects it into `stage/docker`. |
...
| `Docker`                   | Collects every jar from each predecessor's `artifacts/` and `dependencies/` exactly as `Bundle` does, hard-links them into `docker/classpath/` and `docker/modulepath/`, and writes a `docker/Dockerfile` that `COPY`s those folders into the image and starts the module through an exec-form `ENTRYPOINT`. The entry point is read from `launcher.properties`, the same neutral descriptor `Bundle` consumes: a `mainModule` entry yields `--module-path <workdir>/modulepath --module <module>/<class>` (real and automatic-module jars go onto `modulepath/`, the rest onto `classpath/`, mirroring how `Execute` launches), and whenever that graph is not self-contained - an automatic module on the module path, or any class-path jar beside it - the entry point gains `--add-modules ALL-MODULE-PATH` so the whole module path is rooted, the same correction `JPackage` applies through `--java-options` and `Bundle` reports as `selfContainedModuleGraph=false`. A bare `mainClass` instead yields `--class-path <workdir>/classpath/*` and the class name - the JVM expands that wildcard itself, so the exec form needs no shell. The step is skipped when no main launcher is configured or no jar was collected, so it produces nothing for a module without a main class. It never invokes a container tool: the `Dockerfile` and the jars beside it are the whole output, which keeps the build free of a Docker dependency while making the image one `docker build` away. | `artifacts/`, `dependencies/`, `launcher.properties`                                                                          | `docker/Dockerfile`, `docker/classpath/`, `docker/modulepath/`                    |
...
| `docker.properties` | configuration file | An optional file in the configuration location, located exactly like `packaging.properties` (first match wins, so a profile folder's file or a module-local one shadows the project-wide file). Its presence alone activates the feature - there is no boolean key to set: `InferredMultiProjectAssembler` reads it through `Docker.configured(...)` and wires a `docker` step into the package phase that writes a container build context (`docker/Dockerfile` plus the `classpath/`/`modulepath/` jars it copies) for every module declaring a main class (a `@jenesis.main` Javadoc tag, or a `<mainClass>` POM property); modules without one are skipped. The build itself never runs a container tool, so no Docker installation is required to produce the file. Nothing in the emitted file is Docker-specific either: it is plain OCI builder syntax, so `podman build` and `buildah bud` consume the same staged folder unchanged (Podman resolves a `Containerfile` first and falls back to the `Dockerfile` written here, which is why the file keeps that name - it is the one name both builders find without a `-f` flag). Keys, all optional: `from` (the base image, default `eclipse-temurin:<the running Java feature release>-jre`), `workdir` (default `/app`), `user`, `env`, `expose`, `options` (JVM options placed before the entry point) and `arguments` (the `CMD` appended after it). The repeatable keys - `env`, `expose`, `options` and `arguments` - take `\n`-separated values and emit one line or argument each, the same convention `process-<command>.properties` uses for repeated flags. `Inventory` records the context as `prefix.docker`, and the `STAGE` module's `docker` step (`ImageStaging`) collects it into `stage/docker`, so `docker build target/stage/docker/output` is the whole remaining step. Each module's own context is always complete under its `docker/` output; staging merges them into one folder the way `stage/runtime` merges `jlink` images, so a project packaging several main classes at once should build from the per-module output (or stage one module at a time) rather than from the merged folder. |
