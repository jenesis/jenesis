...
`jenesis.project.properties` selects further profiles: a comma-separated list of profile names, each resolved to a
`jenesis-<name>.properties` file (a `.properties` suffix on the name is ignored) relative to the declaring file's folder and loaded the same way. Profiles
compose by *chaining* - any loaded file may set `jenesis.project.properties` itself to pull in more, transitively,
until everything is loaded - so a `release` profile can switch on source jars and an SBOM in one selection. Because a
profile now designates both a `jenesis-<name>.properties` file and a `<name>/` configuration folder (see below), its
properties file is optional: a missing one is simply skipped, so a profile may contribute only a configuration folder.
...
disables it. See the [`profiles`](demo/demo-15-profiles/README.md) demo and *Configuration*.
...
tracing agent, which a `graal.properties` file in the configuration directory switches on. A single build, on a
GraalVM JDK (or with `GRAALVM_HOME` pointed at one), captures that metadata and
compiles the image in one pass once a `packaging.properties` in the configuration location sets `native=true`:

    java build/jenesis/Project.java stage

The `graal.properties` file's presence attaches `-agentlib:native-image-agent` to the test JVM (with
`jenesis.observe.native` a default-`true` suppress override, set it to `false` to switch the agent off), so the
agent records every
...
The `native=true` `packaging.properties` then runs `native-image` over the produced launcher in a terminal *package* phase that runs
...
application image end to end in [`demo-44-native-image`](demo/demo-44-native-image). Committing the metadata under
...
Every resolved dependency is keyed by a **group** - the top isolation axis of dependency resolution,
with scope nested beneath it (`<group>/<scope>/<repository>/<coordinate>`). Each group is a
self-contained version tree: coordinates in different groups resolve and pin independently and never
collide, even when they name the same library at different versions. Two kinds of group exist, and the
defaults are chosen so a normal build never sets either:

- The **project group**, `main` by default, holds the project's own `compile`/`runtime` dependencies.
  Every step or module that consumes them - `Javac`, `Java`, `JLink`, `JPackage`, `NativeImage`,
  `Bundle`, `Launcher`, `TestModule`, the compiler/quality modules' compile classpath, `Inventory`,
  `InternalModule`, `ExternalModule` - takes it from a `group(String)` wither that defaults to `main`.
- A **tool group**, named after the tool (`kotlinc`, `scalac`, `groovyc`, `spotbugs`, `pitest`, …),
  holds that tool's own runtime/plugin closure so it never leaks into the project. The tool modules
  expose a `tool(String)` wither for it, defaulting to the tool name.

Because both are withers over a default rather than hard-coded literals, a custom build can define
**multiple groups**. For example, point two `KotlinCompilerModule`s at different tool groups
(`.tool("kotlinc")` and `.tool("kotlinc2")`, each pinning its own compiler version) to run two
compiler versions side by side, or resolve a second project dependency set under its own group and
drive a packaging step off it with `.group("…")`. The chosen group flows down through the step into
`Dependencies`, which writes it as the leading segment of each index key. The pin grammar abbreviates
the `main` group away (a bare `<module>` or `<groupId>/<artifactId>` is implicitly `main`); any other
group is written with an explicit `<group>/…` prefix.
...
- A **bill of materials** shares a curated set of version and checksum pins across modules and projects in the
  modular layouts. A BOM is a plain properties file whose keys follow the pin token grammar minus the group -
  a bare `<module>` is short for `module/<module>`, a `<groupId>/<artifactId>` (one slash) for
  `maven/<groupId>/<artifactId>`, and anything with more slashes is an explicit `<repository>/<coordinate>` -
  and whose values are the pin value grammar, `<version>[ <algo>/<hash>]`, including leading-colon classifier
  qualifiers; checksums are optional, and platform guards are not allowed inside a BOM file. A module imports a
  BOM through the `@jenesis.bom` tag in `module-info.java`:
  `@jenesis.bom <token> [<version> [<algo>/<hash>]] [<guard>]` names a BOM artifact using the pin token grammar
  (a bare `<module>` is short for `main/module/<module>`, a qualified `<group>/module/<module>` selects the group
  its entries merge into), and the token's repository segment selects the source format through the
  repository's resolver. A `module` token names a BOM properties file that the dependencies step fetches from
  the module repository as `<module>/<version>/<module>.properties` - or, with no version, as the floating
  `<module>/<module>.properties` latest, which is only as reproducible as an unpinned module dependency and
  cannot carry a hash. A `maven` token - the one-slash `<groupId>/<artifactId>` shorthand or a verbatim
  `<group>/maven/<groupId>/<artifactId>` - names a **Maven BOM**: the dependencies step fetches the
  coordinate's pom and imports its effective `<dependencyManagement>` as if it were declared in the root POM,
  with parent chains and properties resolved and nested `<scope>import</scope>` BOMs flattened recursively
  under Maven's first-wins rules (diamond imports are tolerated). The imported entries are keyed
  `<groupId>/<artifactId>[/<type>[/<classifier>]]` beneath the reference's own repository and carry versions
  only: a Maven BOM reference takes no checksum - repositories and mirrors re-serialize POMs, so a byte hash
  over a downloaded pom is not stable - and `Checksum/` comments inside a fetched BOM are never read or
  trusted. Managed exclusions and scopes are dropped, and a managed entry without a version (an
  exclusions-only entry) is skipped. A versionless Maven reference negotiates the latest release through the
  repository metadata, with the same reproducibility caveat as a floating module BOM. A token of
  `[<group>/]pin-<name>.properties` instead reads that file from the project's BOM locations: the `boms`
  property of `Project` (`-Djenesis.project.boms=<path-separated>` or the `boms(...)` builder method), which
  defaults to the `configuration` locations - by default `build.jenesis/` under the project root - so one
  shared file serves every module. The `pin-` prefix names what the file holds - pins - and keeps a file reference structurally distinct
  from a module coordinate, since a dash can never occur in a Java module name (the same fact the classifier
  fusion relies on), while staying clear of the `bom.properties` build-configuration file that gates BOM emission. The locations are
  searched in order with the first hit winning, and they are deliberately not profile-resolved, so no profile
  can swap a BOM and a build cannot come to depend on dynamic configuration. Every `pin-*.properties` file found
  in the BOM locations is bound as a tracked input of each module's manifests step, so editing a BOM invalidates
  the build correctly. The declaration -
  not the BOM file - carries the group and an optional trailing platform guard with the same matching rules as
  pins (most specific guard wins, the unguarded line is the fallback, an unmatched guard skips the import).
  Precedence is strictly local-first: an explicit `@jenesis.pin` always overrides a BOM entry, and when several
  BOMs pin the same coordinate the last declared wins - a later BOM overrides an earlier one, so broad curation
  is declared first and local refinement last. Under the pinning modes a BOM behaves like its pins:
  `strict` requires a checksum on a module BOM reference (the fetched file is content-verified before its
  entries are trusted) and accepts BOM-provided hashes as pins for the closure check - a Maven BOM reference is
  exempt, since it cannot carry one; its parent and nested pom content is fetched unverified, exactly like
  every pom walked during Maven resolution, and reproducibility comes from the pin step pinning the artifacts
  resolved through it instead - `versions` keeps the pinned versions but skips all checksum verification, and `ignore`
  floats the BOM itself: a versioned reference is re-fetched at the latest published BOM where the repository
  can discover it (a Maven reference through the repository metadata, a module reference through its floating
  discovery pom in the Maven-backed layout; the pure modular jar repository keeps the declared version), and
  the latest BOM's entries keep managing resolution while explicit pins float - so a pin that deliberately
  overrode the BOM snaps back to the BOM's curation on a repin. Beyond the BOM entries, `ignore` floats
  everything except classifier qualifiers and the managed version of a dependency whose declaration carries no
  version of its own (there the management is the only version source, so dropping it could only break
  resolution, never float it - the test engine's `junit-platform-console`, versioned solely through
  `dependencyManagement`, is the archetype). Manifests writes the declarations into a per-module `boms.properties`
  (`bom/<group>/<repository>/<coordinate>` references to fetch, `entry/...` lines already expanded from local
  files), which merges across modules and feeds the resolution pass beneath `versions.properties`. Publishing a
  BOM is a matter of placing the properties file at the module repository's standard versioned path; the local
  `~/.jenesis` repository is consulted first, so a locally exported BOM resolves before the remote one. A module
  emits such a BOM automatically when a `bom.properties` build-configuration file is present: the modular layouts
  render the module's full resolved closure (a version and checksum per coordinate, keyed group-less like a
  hand-written BOM) and `export` publishes it into the Jenesis repository as `<module>/<version>/<module>.properties`
  next to the module jar, ready to be consumed by `@jenesis.bom <module>` elsewhere. The emission is gated on the
  file's presence, mirroring how `packaging.properties` gates `jpackage`, and is confined to the Jenesis
  repository - the Maven export never carries it.
  The `pin` goal is BOM-aware along two independent dimensions, `-Djenesis.pin.bom=keep|flatten` and
  `-Djenesis.pin.checksum=true|false`. Under `keep` (the default) a dependency whose resolved version - and
  checksum, when checksums are pinned - is already supplied by a module BOM entry gets no `@jenesis.pin` line,
  and an existing unguarded line for such a covered coordinate is removed as redundant (a pin that genuinely
  overrides the BOM forces a different resolution, so it is never "covered" and is refreshed as usual); the BOM
  itself is pinned instead: every versioned module `@jenesis.bom` reference has its content hash written back
  onto the declaration (guard matching selects the line to refresh exactly as for pins), while floating
  references stay floating and local files need no pinning. A Maven BOM inverts this: its entries never cover,
  because they cannot seal content - the outer pom's hash would not fix the parent and nested pom bytes its
  expansion depends on - so every artifact the resolved closure takes from the BOM is pinned as usual, version
  plus the hash computed over the downloaded artifact, and the declaration itself is refreshed with the
  resolved version alone, no hash - it remains the durable upgrade anchor that a repin under `ignore` floats
  to the latest release and re-pins. Entries outside the closure are deliberately not materialized - they
  would only be noise - and keep managing resolution from the fetched BOM; under strict pinning an artifact
  newly entering the closure through them fails closed until the next `pin` run records it. Under `flatten`
  the unguarded `@jenesis.bom` declarations are removed and the module's resolved closure is pinned in full -
  the inverse migration - with platform-guarded
  BOM declarations failing the flatten, since a local repin only observes the local platform's variant.
  `-Djenesis.pin.checksum=false` switches every written value (pins and BOM references alike) to versions
  without hashes, and coverage under `keep` then requires only a version match. The resolved BOM data reaches
  the pin step the same way the dependency closure does: `Dependencies` records the fetched BOM files, the
...
- **Module aliases** (`MODULAR_TO_MAVEN` layout only) give a Maven artifact a stable module name of the
  ...
  derives exactly that automatic module name. `resolved/` holds hard links into the shared artifact cache, so the
  cache is untouched and no jar is ever rewritten: signature entries survive and a pinned checksum keeps
  describing the jar that is actually on the command line. A `.module` sidecar written beside the renamed jar
  records the intent, so this build places it on the module path while a jar whose file name merely happens to
  derive a legal module name stays a class path entry as before.
  An alias does not have to be required, and does not define anything: it names any otherwise class-path jar in
  the tree, including a transitive dependency the project never mentions, which is enough to make it a module
  ...
  is an error wherever the two declarations come from: one name claimed for two artifacts, two names claimed for
  one artifact, or a name a resolved module already carries. A target that already carries module identity is
  rejected as well, whether it declares a `module-info` or only an `Automatic-Module-Name` - it is addressable
  under that name, so require it directly. Because the target is now a real automatic module, a package it
  shares with another module, or a class it keeps in the default package, is reported by `javac` and `java`
  themselves, as for any other module path entry. An alias is an automatic module, and an automatic module is not
  linkable, so `jlink` and modular `jpackage` images cannot consume one directly - a `modules` `packaging.properties`
  key turns the whole closure, aliases included, into explicit named modules for the package phase and makes them
  linkable (the `modules.properties` row of the *Configuration* table below).
...
- **Dependency exclusions** (`MODULAR_TO_MAVEN` layout only) drop a transitive dependency an upstream pom
  should not have declared: `@jenesis.exclude <module> <groupId>/<artifactId>...` in `module-info.java` removes
  those artifacts from what requiring `<module>` resolves. One line names one module and any number of targets,
  and repeated lines for the same module add to each other rather than conflict, so a growing list of upstream
  mistakes stays readable. Each target is an exact `<groupId>/<artifactId>` with no version and no type or
  classifier: an exclusion names an artifact, not one of its variants, and a version slot would only invite
  the question of what a mismatch means. Excluding from a module the declaring `module-info.java` does not
  `requires` is an error rather than a silent no-op, since it is a typo in every case that matters.
  The mechanism is Maven's own. A `requires`d module resolves through `MavenModuleResolver`, which fetches the
  module's discovery pom and hands it to the Maven resolver as the root of the traversal; the declared
  exclusions ride along as that root dependency's `<exclusions>`, so an excluded artifact takes **the whole
  subtree it pulled in** with it, exactly as it would in a hand-written pom. Because the artifact never enters
  the resolved closure, there is nothing left to leak: it is absent from the compile and runtime paths, from
  the tests, from the linked image, and from the generated pom, which in this layout is written from the
  resolved closure. It is likewise absent from `sbom` and the compliance checks, which is the honest answer -
  the build genuinely never fetched it. `ModularProject` writes the declarations into `exclusions.properties`
  keyed exactly as the requirement they apply to, so an exclusion for an aliased module follows the alias to
  the Maven coordinate it names. In the purely modular layout the declaration is rejected: resolution there
  reads module descriptors from the module repository and never a pom, so there is no transitive pom
  dependency to exclude.
- **Java agents** are declared with `@jenesis.attach <token> [<arguments...>]` in `module-info.java`, or with the
  same lines in a project-level `<!--jenesis.attach ... -->` comment block in `pom.xml` (a direct child of
  `<project>`, one declaration per line, read only from the project's own POM like the `<!--jenesis.pin ... -->`
  block; `&#45;&#45;` escapes a double dash, which XML forbids inside comments). Every declared library is added
  as a `-javaagent:` to the java commands of the declaring module: an `Execute` run of its `@jenesis.main` class
  (agents travel through `inventory.properties` as `prefix.agent.<n>` entries) and its test executions, where the
  attach agents follow any observability agents such as JaCoCo, in declaration order. The token follows the pin
  token grammar without a version slot - the version comes from a declared dependency, a `@jenesis.pin`, or a
  BOM entry, and floats latest without any - and everything after the token is passed verbatim as the agent
  options after `=`, so an option string may contain single spaces (runs of whitespace collapse, and tabs or
  newlines cannot be expressed). Both agent shapes work through the one construct: an **agent-only** library
  (the OpenTelemetry javaagent) resolves in its own `agent` dependency scope and never joins a compile or
  runtime path, while a **dependency that attaches** (Mockito) names a coordinate that is also required, and
  both scopes resolve in the same pass to the identical artifact, so the attached jar is byte-for-byte the jar
  on the class or module path. In the Maven layout an attach that matches a declared dependency follows its
  scope: a test-scoped match attaches to the test runs only, any other match and an unmatched agent-only line
  attach to both the main and the test module. The declaration is scoped to the module that carries it and never
  propagates to dependents; the resolved agent jar must carry a `Premain-Class` manifest attribute, which is
  validated with a clear error before launch. Agent classes load through the system class loader outside the
  module graph (for a dual-use coordinate the named module on the module path wins for class loading -
  Mockito's documented setup), and only the declared coordinate is attached: its transitive dependencies
  resolve into the `agent` scope for inspection (SBOM, reports) but are not themselves attached.
...
- The **inferred code-quality chains** bring the same zero-config activation to static-analysis and formatting
  ...
  for scalafmt, and `codenarc.xml` for CodeNarc. Each trigger file is also passed to its tool as the active
  configuration - for ktlint as `--editorconfig`, so the located file's properties apply as defaults no matter
  where it lives (an `.editorconfig` sitting beside the sources still wins for the properties it sets, matching
  editorconfig semantics). Like the compilers, each tool resolves in its **own dependency
  ...
  [`pitest`](demo/demo-25-pitest/README.md) demo shows it killing both mutants of a covered method. The detekt runner targets the
...
- The **inferred formatting chain** (`InferredSourceFormattingModule`) is the rewriting counterpart to the
  ...
  them in place. It covers `google-java-format` and `palantir-java-format` for Java (selected with a
  `javaformat.properties` file naming `google` or `palantir`), `ktlint -F` for Kotlin, and `scalafmt` for Scala. The
  assembler wires this chain by default too: the Java formatter activates from a `javaformat.properties` file the
  same way ktlint and scalafmt activate from their linter config files (`.editorconfig`, `.scalafmt.conf`), and a
  formatter runs whenever its sources are present and self-skips otherwise. Each of the three switches off with
  `jenesis.format.java` / `jenesis.format.ktlint` / `jenesis.format.scalafmt` (each default `true`). Each tool resolves in its own dependency group, floats `RELEASE`, and runs in a forked JVM; the two
...
Build plug-ins activate on the **presence** of a configuration file, and every lookup walks one ordered folder
list; the first folder that carries the file wins (only `spdx.properties` merges across all of them). The list
is composed of the per-module locations, most specific first, followed by the project-wide locations:

| Rank | Location | Layouts | Scope |
|---|---|---|---|
| 1 | `src/main/build.jenesis/` or `src/test/build.jenesis/` under the pom root | MAVEN | the pom's main or its test module alone |
| 2 | `build.jenesis/` next to the `pom.xml` | MAVEN | both of the pom's modules |
| 1 | `META-INF/build.jenesis/` under the module sources | MODULAR, MODULAR_TO_MAVEN | the one module (test modules are separate modules with their own folder) |
| 3 | the `jenesis.project.configuration` folders | all | project-wide, defaulting to `build.jenesis/` under the project root |

Profile subfolders (`<folder>/<profile>/` for each active `jenesis.project.properties` profile) rank before all
plain folders, so an active profile's folder beats even a module's unscoped one. The bare project root is
deliberately not a default location: a conventionally named file that an editor or a teammate drops at the root
(an `.editorconfig`, a `checkstyle.xml` for the IDE) must not silently alter the build - configuration activates
only from an explicitly named `build.jenesis/` folder, or from locations the project opts into via
`jenesis.project.configuration`.

What lives in these folders - presence activates, contents configure:

- **Packaging and output**: `packaging.properties` (jmod, jlink, jpackage, native), `modules.properties`, `sbom.properties`.
- **Code quality** (the tool is inferred from its file): `checkstyle.xml`, `pmd.xml`, `spotbugs-exclude.xml`,
  `detekt.yml`, `codenarc.xml`, `scalastyle-config.xml`.
- **Formatting**: `javaformat.properties` (google or palantir), `.editorconfig` (ktlint), `.scalafmt.conf`.
...
A few locations sit deliberately outside this search: `[<group>/]pin-<name>.properties` BOM files resolve from
the `jenesis.project.boms` locations (defaulting to the configuration locations) and are never profile-resolved;
the `jenesis.properties` cascade consists of the project root file, the user-global
`~/.jenesis/jenesis.properties` and `jenesis-<name>.properties` profile files resolved relative to whichever
file declares them (precedence: explicit `-D` over profiles over the project file over the user-global one);
`jenesis.project.metadata` names explicit files rather than searched folders; and pins live in the sources
themselves, as `@jenesis.pin`/`@jenesis.bom` tags in `module-info.java` or `<!--Checksum/...-->` comments in a
...
off. The [`test-selection`](demo/demo-24-test-selection/README.md) demo shows it editing one class and re-running only
that class's test.

### Printing the dependency tree

Run the `dependencies` selector to print each module's dependency tree, the way `mvn dependency:tree` /
`gradle dependencies` do:
...
resolver returns a `Resolution` in a single pass: the flat materialized artifacts, the parent -> child resolution
graph (each `Edge` carries the property-file key, the discovered version and scope, and a `followed` flag that is
`false` on a dedup re-encounter), and a `Vertex` per resolved coordinate (its negotiated version, module name and
automatic flag, and declared licenses). The `Dependencies` step flattens the artifacts into
`dependencies.properties` and writes the graph to `graph.properties` and the dependency licenses to
`licenses.properties` beside it on every resolve. As it writes them it **normalizes and classifies** each
declared license: the declared name is matched case-insensitively against a comprehensive built-in alias table to
a canonical SPDX identifier (`Apache-2.0`, `EPL-2.0`, ...), and that identifier against a category table
(`permissive`, `weak-copyleft`, `strong-copyleft`, `network-copyleft`, `public-domain`); both are stored in the
sidecar alongside the raw name and url, so every downstream consumer reads the same normalized result rather than
re-deriving it. The two tables can be extended through `spdx.properties` files in the configuration location
(resolved through the same `configurations(...)` search as the other config files, so every project, profile and
per-module folder that carries one contributes and all of them merge). The located files are bound per module and
travel as a `ProjectModule.spdx()` key-set - a first-class descriptor input alongside `sources`/`manifests`/
`artifacts`, propagated exactly the way `versions.properties` rides `sources()`. That means the extended tables
reach **every** `Dependencies` step, not just the top-level dependency tree: the per-module resolves and the
assembler's own resolves (`InternalModule`, `TestModule`, the tool/compiler/documentation modules) all classify
against the same tables. A file uses one prefixed key space so the two kinds of entry stay connected:
`alias/<declared name>=<SPDX id>` **appends** an alias, `category/<SPDX id>=<category>` **appends** a
classification (standard properties, spaces in a key escaped as `\ `); supplying one is rarely necessary. The
name is distinct from the compliance `licensing.properties` (the `allowed`/`denied` policy), which is a separate
concern.

The `Inventory` step surfaces each module's `graph.properties` and `licenses.properties`, and the `dependencies`
step (`Tree`) reads them back, reconstructs the graph per `<group>/<scope>`, and renders it through a
`DependencyTreeReport`. Each node shows the version each parent requested, the negotiated version inline when it
differs (`[1,2] -> 2`), the Maven scope, any module metadata, and the normalized license identifier inline
(`{Apache-2.0}`, falling back to the raw declared name when unrecognized); not-followed duplicates are dimmed and
marked `(*)`, and a per-declared-dependency colour gradient tints the tree connectors. A `Resolved dependencies`
list of negotiated versions follows each tree. For `MODULAR_TO_MAVEN`, each module's resolved Maven coordinate is
what the tree shows (e.g. the `org.slf4j` module resolves to `maven/org.slf4j/slf4j-api/2.0.16`).

Below the trees the step prints a colour-coded **summary** over the project's distinct resolved dependencies. It
opens with a tally of how many distinct licenses the project implies and, when any dependency declares more than
one, how many do. Then a `Licenses` distribution (each SPDX identifier with its count, percentage and a
proportional bar, coloured by category) - a dependency that offers several licenses is counted under its **most
permissive** one; a `Permissiveness` distribution that rolls those choices up by category
(`permissive`/`weak-copyleft`/`strong-copyleft`/`network-copyleft`/`public-domain`/`unknown`, most to least
permissive); and a `Modules` breakdown of how many dependency jars are `named` (explicit) modules, `automatic`
modules, or `non-modular` plain jars.

The same `graph.properties` feeds the SBOM: the `Sbom` step - wired per module by `InferredMultiProjectAssembler`
by default, the format chosen by an optional `sbom.properties` (`format=json|xml`, `format=none` to disable) and
suppressed by `-Djenesis.sbom.cyclonedx=false` - reads it and emits the CycloneDX `dependencies` array
(`ref`/`dependsOn`, each component carrying a `bom-ref`) alongside the components and licenses, so the emitted
bill of materials records the resolved dependency relationships, not just a flat component list. Because the
licenses were already normalized at extraction, the `Sbom` step and its `CycloneDx` emitter carry no license
logic of their own: each license is emitted as a CycloneDX `license.id` when the stored identifier is a
recognized SPDX id (a default set, overridable through the `CycloneDx.identifiers(...)` wither) and otherwise as a
free-text `license.name` with url. The document is
embedded in the jar (`META-INF/sbom/`), collected as a `stage/reports/sbom/` report, and attached to a staged
Maven repository as `<artifact>-<version>-cyclonedx.<ext>` for publication - see the [`sbom`](demo/demo-12-sbom/README.md) demo.
...
Resolution reaches the same repositories a build does, with no configuration: `MavenDefaultRepository.of()`
for Maven coordinates, and one `JenesisModuleRepository` for module names - `Scope.ARTIFACT` for the POM
discovery of the default mode, `Scope.MODULE` for `--modular`, both of which read the local `~/.jenesis`
before the public repository. Which of the two a `Jpx` carries is fixed when it is constructed
(`new Jpx(modular)`), so
an instance holds exactly one module repository and one module resolver rather than a pair to pick from at
install time. `MAVEN_REPOSITORY_URI` and `JENESIS_REPOSITORY_URI` (or `-Djenesis.maven.uri` and
`-Djenesis.module.uri`) redirect them at a mirror or an internal repository, exactly as they do for a build.

The [`jpx`](demo/demo-46-jpx/README.md) demo runs the JUnit Platform Console Launcher both ways - by module
name and by Maven coordinate - each pinned to a version and verified against an installation hash, then once
more against a 32-character prefix of that hash and once against a hash that does not match and is blocked.
It drives the API rather than the command line: `install` takes the target as a string
(`jpx.install("org.junit.platform.console@6.1.3").verify(<hash>).launch(List.of("--version"))`), with
`install(Jpx.Command)` underneath for a target that is already parsed.
...
| `analyzed/`                | `JDeps.ANALYZED`                 | A flat directory of the jars a `JDeps` step is to generate a module descriptor for, each already named `<module>.jar` after the module it is to become - `jdeps` derives the generated module's name from the file name, so the rename is what fixes the name. Written by `ModularizeModule`'s `prepare` step as hard links into the shared artifact cache, so no jar is rewritten in place. |
| `modules/`                 | `JDeps.MODULES`                  | A flat directory of the jars that already carry an explicit `module-info`, passed to `jdeps` as `--module-path` so the analyzed jars resolve their references against them. The JDK's own modules are observable without staging, exactly as for `jlink`. Written by `ModularizeModule`'s `prepare` step. |
| `modular/`                 | `Dependencies.MODULAR_PATH`      | A flat directory of **explicit named module jars** written by `ModularizeModule`'s `modularize` step, one per entry of the closure it rewrote: already-explicit modules hard-linked through byte for byte, everything else a copy carrying a synthesized `module-info.class`. It sits beside the resolved closure rather than replacing it, and is reached only through the `modular.properties` index. |
| `descriptors/`             | `JDeps.DESCRIPTORS`              | The `module-info.java` sources `jdeps --generate-module-info` produced, one folder per generated module (`descriptors/<module>/module-info.java`, or `descriptors/<module>/versions/<n>/module-info.java` when a multi-release version was selected). They are an intermediate: `ModularizeModule`'s `modularize` step reads the `requires` directives out of them and emits the `module-info.class` itself, since `jdeps` cannot write class files. |
...
| `runtime/`                 | `JLink.RUNTIME`                  | Conventional output location for a custom runtime image produced by the `jlink` tool via the `JLink` step (a self-contained JDK image tree with `bin/`, `lib/`, `release`, and so on). When the assembler wires it (via a `jlink=true` `packaging.properties`), `Inventory` records it as `prefix.image` and the `STAGE` module's `runtime` step (`ImageStaging`) collects it into `stage/runtime`.                                                                |
| `native/`                  | `NativeImage.NATIVE`             | Conventional output location for the standalone executable produced by the `native-image` tool via the `NativeImage` step, named after the module's `--name`. When the assembler wires it (via a `native=true` `packaging.properties`), `Inventory` records it as the scalar `prefix.native` and the `STAGE` module's `native` step (`ImageStaging`) collects it into `stage/native` - the native-image analogue of `stage/packages` (jpackage) and `stage/runtime` (jlink).                                                                |
| `jmods/`                   | `JMod.JMODS`                     | Conventional output location for `.jmod` module files produced by the `jmod` tool via the `JMod` step, named `<module>.jmod`. A `.jmod` is the module's own produced binary but, unlike a modular jar, is valid only on a compile- or link-time module path and never at runtime, so it lives apart from `artifacts/` (whose contents are walked onto the runtime path unfiltered) and is consumed by the `JLink` step as `--module-path` entries. When the assembler wires it (via a `jmod=true` `packaging.properties`), `Inventory` records it as `prefix.jmod` and `ModularStaging` hardlinks it alongside the jar in `stage/modular`. Because it is staged next to the jar, the `JenesisModuleRepository` resolves a `<module>:jmod` coordinate qualifier to the published `.jmod` (analogous to `:pom`), falling back to the jar when no `.jmod` was published - so a consumer (e.g. a custom assembler wiring cross-module link-time propagation) can request the link-time form unconditionally. |
...
| `docker/`                  | `Docker.DOCKER`                  | Conventional output location for the container build context produced by the `Docker` step: a `Dockerfile` next to the `classpath/` and `modulepath/` folders it copies into the image. Produced only for modules that declare a main class. Nothing in the build runs a container tool - the folder is the input a later `docker build` (or `podman`, or a remote builder) consumes. When the assembler wires it (via a `docker=<base image>` line in `packaging.properties`), `Inventory` records it as `prefix.docker` and the `STAGE` module's `docker` step (`ImageStaging`) collects it into `stage/docker/<module>`, a folder per image. Unlike a package or a native binary, which are already named for their module and so stage flat, a container context is a directory rooted at a fixed-name `Dockerfile`: two staged side by side would merge, and merge silently. |
...
| `boms.properties`          | `BuildStep.BOMS`                 | Sibling of `versions.properties` written by `ModularProject.Manifests` when a `module-info.java` carries `@jenesis.bom` tags. Two key shapes share the file so declaration order (and with it last-declared-wins precedence) survives: `bom/<group>/<repository>/<coordinate>=<version>[ <algorithm>/<hex>]` is a repository BOM reference (an empty value floats to the latest published BOM), and `entry/<group>/<repository>/<coordinate>=<version>[ <algorithm>/<hex>]` is an already-expanded entry from a local BOM file (the `[<group>/]pin-<name>.properties` declaration form naming a file in the project's BOM locations - `jenesis.project.boms`, defaulting to the configuration locations - expanded at manifests time; each found `pin-*.properties` is bound as a tracked manifests input, and the locations are fixed and never profile-resolved). Platform guards on `@jenesis.bom` declarations are already resolved when this file is written, exactly like guarded pins in `versions.properties`. `MultiProjectDependencies` merges the per-module files with `putIfAbsent`; `Dependencies` fetches each `bom/` reference through the repository's resolver (`Resolver.bom`, with the properties default fetching `<coordinate>[/<version>]:properties` and the Maven resolver flattening the coordinate's pom `<dependencyManagement>`), validating a module reference's own checksum unless pinning is `versions`/`ignore` and requiring one under `strict` for non-internal items (a Maven reference cannot carry one and is exempt), and folds all entries beneath `versions.properties` pins via `putIfAbsent` - so an explicit `@jenesis.pin` always wins over any BOM entry, except under `ignore`, where the reference floats to the latest published BOM and its entries outrank the ignored pins. `Dependencies` also writes a **resolved** `boms.properties` into its own output for the `pin` step: `bom/<group>/<repository>/<coordinate>/<version>=<relative path of the fetched properties file>` per versioned module reference, `version/<group>/<repository>/<coordinate>=<version>` per versioned Maven reference (no file to hash), and the merged `entry/...` map from properties BOMs and local files (coverage semantics; Maven BOM entries are deliberately absent, so they never cover and the closure coordinates they manage are pinned like any other resolved artifact); `Inventory` mirrors them as indexed `prefix.bom.<n>` entries (read only from folders that also carry `dependencies.properties`), which is how `PinModuleInfo` learns what a BOM supplies and where to hash a BOM file from. |
| `exclusions.properties`    | `BuildStep.EXCLUSIONS`           | Sibling of `requires.properties` produced by `MavenProject.Manifests` (when a dependency declaration in a `pom.xml` carries `<exclusions>`) and by `ModularProject.Manifests` (from a `@jenesis.exclude <module> <groupId>/<artifactId>...` declaration in `module-info.java`, keyed against the very coordinate that module resolves under, so an exclusion for an aliased module follows the alias to its Maven coordinate). Each key is a `<group>/<scope>/<repository>/<coordinate>` from `requires.properties`; the value is a comma-separated list of `<groupId>/<artifactId>` patterns that the resolver must subtract from this dependency's transitive closure (so e.g. `mockito-core <exclusion>net.bytebuddy/byte-buddy</exclusion>` does not silently re-pull `byte-buddy` through the test classpath). `MultiProjectDependencies` carries the entries through to the per-scope prepare step alongside the matching `requires.properties` rows; `Dependencies` reads the file from its arguments and threads the exclusion set per coordinate into `Resolver.dependencies`, where `MavenPomResolver` populates `MavenDependencyValue.exclusions` so the transitive walk honours them. `MavenModuleResolver` carries the set into the root `MavenResolver.RootPom` it builds for a `requires`d module, which becomes that root dependency's `MavenDependencyValue.exclusions`, so an excluded artifact drops out together with the subtree it pulled in. `ModularJarResolver` rejects any non-empty exclusion set up front, since a purely modular resolution reads module descriptors and never a pom, leaving no transitive pom dependency to exclude. `Pom` reads the file to emit each `<dependency>` with its declared `<exclusions>`, so consumers of an unresolved POM keep the same closure. A *resolved* POM (the one `MODULAR_TO_MAVEN` emits) states the flattened closure instead, and so excludes everything from every dependency: each artifact is already listed in its own right, walking a subtree could only re-derive it from today's POMs rather than the ones this module resolved against, and it is what let an exclusion leak - the artifact was gone from this module's closure but a consumer walked the subtree it had been excluded from and pulled it back. `MavenPomResolver` reads the same `*`/`*` as licence to skip fetching those POMs at all. The file is omitted entirely when no dependency in the module declares exclusions. |
| `attachments.properties`   | `BuildStep.ATTACHMENTS`          | Sibling of `requires.properties` written by the manifests steps when a module declares `@jenesis.attach` tags in `module-info.java` (`ModularProject.Manifests`) or its `pom.xml` carries a project-level `<!--jenesis.attach ... -->` comment block (`MavenProject`'s manifests step, scope-following: a declaration matching a test-scoped dependency reaches only the test variant's file, any other match and an unmatched agent-only line reach both). Each key is the agent-scoped coordinate `<group>/agent/<repository>/<coordinate>` (always version-less; the matching `requires.properties` seed carries a trailing `/<version>` in the Maven layout when a declared or managed dependency supplies one, so a dual-use coordinate resolves to the same version - and through the shared resolution pass to the identical artifact file - as its class-path twin), each value the verbatim agent options string (possibly empty). Consumers join the keys against `dependencies.properties` by exact match or one trailing `/<version>` segment (never more, so transitive classifier variants cannot be mistaken for the declared agent): `TestModule.Run` composes `-javaagent:<jar>[=<options>]` lines after any observability agents, and `Inventory` emits the `prefix.agent.<n>` entries that `Execute` turns into the same flags for `@jenesis.main` runs. Only the declared coordinate attaches (its transitives resolve into the `agent` scope but are not attached), and the joined jar must carry a `Premain-Class` manifest attribute - validated with a clear error before launch. |
...
| `.jenesis.lock`           | `BuildExecutor.LOCK_MARKER`     | An empty file at the same root carrying an exclusive OS file lock for the duration of the build process. A second build process on the same target fails fast with a clear error instead of the two silently deleting each other's staged step folders; the lock dies with the process, so a crashed build never leaves a stale lock. One process may build the same target repeatedly (watch mode, embedded executors). |
| `<step>~/`                | staging convention               | A running step's staging folder: the executor stages `output/` and `supplement/` under the step's encoded name with a tilde suffix and atomically renames the folder onto the final name when the step succeeds, so a half-written step can never masquerade as a completed one. The tilde cannot collide with a real step folder since `URLEncoder` never emits one. A staging folder left behind by a crashed run is replaced when the step next executes. Deterministic naming keeps the recorded tool command lines, and with them the whole target tree, byte-for-byte reproducible. |
...
| `JLink`                    | Collects the modular jars and `.jmod` files in each predecessor's `jmods/`, `artifacts/` and `dependencies/` into a `--module-path` and invokes the `jlink` tool with `--output runtime/`, writing the produced custom runtime image to `runtime/`. The JDK's own modules are observable to `jlink` automatically, so only the project's modules need staging. The root modules to link in (`--add-modules`) and any other options are supplied through a `process/jlink.properties` file, the same way `Javac` reads `--release` from `process/javac.properties`. The step is skipped when no `--add-modules` is configured (so it does not run for non-modular modules). When `InferredMultiProjectAssembler` wires it (via a `jlink=true` `packaging.properties`), the `prepare` step supplies `--add-modules <module>`, `Inventory` records the image as `prefix.image`, and the `STAGE` module's `runtime` step (`ImageStaging`) collects it into `stage/runtime`. | `jmods/`, `artifacts/`, `dependencies/`, `process/jlink.properties`                                                                   | `runtime/`                                                                       |
| `JMod`                     | Packs each predecessor's `classes/` into a single `jmods/<module>.jmod` via the `jmod` tool's `create` mode, naming the file after the module read from `classes/module-info.class`. It also routes a predecessor's `jmodconfig/`, `jmodlibs/` and `jmodcmds/` folders to `jmod --config`/`--libs`/`--cmds`, so a custom step can pack non-class content (config files, native libraries, commands) that `jlink` then lays into a runtime image's `conf/`/`lib/`/`bin/` - content a jar cannot carry into a runtime (see the [`custom-jmod`](demo/demo-35-custom-jmod/README.md) demo). The output goes to `jmods/` rather than `artifacts/` because a `.jmod` is a link-time-only artifact (rejected on the runtime module path), and the path consumers walk `artifacts/` unfiltered; `jmods/` keeps it off the runtime path while still feeding `JLink`. Additional options (`--main-class`, `--module-version`, and so on) are supplied through a `process/jmod.properties` file, the same way `Javac` reads `--release` from `process/javac.properties`. When the classes carry no `module-info.class` the step is skipped. When `InferredMultiProjectAssembler` wires it (via a `jmod=true` `packaging.properties`), `Inventory` records the file as `prefix.jmod` and `ModularStaging` hardlinks it alongside the jar in `stage/modular`. | `classes/`, `jmodconfig/`, `jmodlibs/`, `jmodcmds/`, `process/jmod.properties`                                                                                                | `jmods/<module>.jmod`                                                            |
| `JDeps`                    | Runs the `jdeps` tool **once** over every jar staged in a predecessor's `analyzed/` folder, resolving their references against the modules staged in `modules/` (handed over as `--module-path`) and the JDK's own modules, and writes the generated sources to `descriptors/<module>/`. One invocation covers the whole set, so the analyzed jars may reference each other and `jdeps` resolves those edges itself rather than needing a call per jar. Each analyzed jar must already be named after the module it is to become, because `jdeps` derives the generated module's name from the file name (or an `Automatic-Module-Name` manifest entry) - which is what `ModularizeModule`'s `prepare` step arranges. Beyond those three arguments the step invents nothing: `jdeps` runs on its own defaults, and every further option arrives through a `process/jdeps.properties` file, the same way `Javac` reads `--release` from `process/javac.properties` and `JLink` reads `--add-modules` from `process/jlink.properties`. The three it does own are the ones no properties file can state, because they are the build graph: what to analyze, what to resolve against, and where its own output folder is. The step is skipped when nothing is staged for analysis. `jdeps` only ever emits Java **sources**; turning them into a `module-info.class` is the caller's job.                                       | `analyzed/`, `modules/`, `process/jdeps.properties`                                                                                   | `descriptors/<module>/[versions/<n>/]module-info.java`                           |
| `Bundle`                   | Collects every jar from each predecessor's `artifacts/` and `dependencies/` and writes a single `bundle/bundle.zip` containing two folders, `classpath/` and `modulepath/`, plus an `application.properties` with a `mainClass` property (and, for a modular launcher, a `mainModule`, plus - whenever the module graph does not resolve on its own - a `javaOptions` entry holding the space-separated JVM options that correct it, which the consumer splices in verbatim: `--add-modules=ALL-MODULE-PATH,ALL-DEFAULT` when the module path carries an automatic module or there are `classpath/` jars, so that the whole module path and the default platform set are rooted. The entry is absent when the launched module's `requires` closure covers the module path). The launcher is read from `launcher.properties`, the neutral entry-point descriptor the assembler's `prepare` step writes for every artifact step (rather than `jpackage`'s own `process/jpackage.properties`, so no artifact step depends on another's command file): a `mainModule` entry yields a modular bundle (real and automatic-module jars go onto `modulepath/`, the rest onto `classpath/`, mirroring how `Execute` launches), while a bare `mainClass` yields a class-path-only bundle. The step is skipped when no main launcher is configured, so it produces nothing for a module without a main class. The zip is a ready-to-run input for building a Docker image or similar: the consumer puts `modulepath/` on `--module-path`, `classpath/` on `--class-path`, and reads the entry point from `application.properties`. | `artifacts/`, `dependencies/`, `launcher.properties`                                                                          | `bundle/bundle.zip`                                                              |
| `Docker`                   | Collects every jar from each predecessor's `artifacts/` and `dependencies/` exactly as `Bundle` does, hard-links them into `docker/classpath/` and `docker/modulepath/`, and writes a `docker/Dockerfile` that `COPY`s those folders into the image and starts the module through an exec-form `ENTRYPOINT`. The entry point is read from `launcher.properties`, the same neutral descriptor `Bundle` consumes: a `mainModule` entry yields `--module-path /app/modulepath --module <module>/<class>` (real and automatic-module jars go onto `modulepath/`, the rest onto `classpath/`, mirroring how `Execute` launches), and whenever that graph is not self-contained - an automatic module on the module path, or any class-path jar beside it - the entry point gains `--add-modules ALL-MODULE-PATH,ALL-DEFAULT` so the whole module path and the default platform set are rooted, the same correction `JPackage` applies through `--java-options` and `Bundle` reports as a `javaOptions` entry. A bare `mainClass` instead yields `--class-path /app/classpath/*` and the class name - the JVM expands that wildcard itself, so the exec form needs no shell. The step is skipped when no main launcher is configured or no jar was collected, so it produces nothing for a module without a main class. The base image is the step's only input beyond the module itself, taken from the `docker` key's value; `/app` and the entry point are fixed, since a Dockerfile inherits `ENV`, `WORKDIR`, `USER` and `EXPOSE` from its base, so image environment belongs in a base image rather than in build configuration. It never invokes a container tool: the `Dockerfile` and the jars beside it are the whole output, which keeps the build free of a Docker dependency while making the image one `docker build` away. | `artifacts/`, `dependencies/`, `launcher.properties`                                                                          | `docker/Dockerfile`, `docker/classpath/`, `docker/modulepath/`                    |
| `NativeImageMetadata`     | Routes captured GraalVM reachability metadata to the module being imaged, so a multi-module build never feeds one image another module's capture. Reads this module's own inventory (the only input outside the `SELECTION` namespace the package phase wires) for its `prefix.artifact`, then, among the sibling inventories, collects the `nativeimage` capture of the test module whose `prefix.test` names that artifact into a single `native-image/` config directory the `native-image` step discovers by content. A main module has at most one test module (staging rejects more), so there is one source and nothing to merge; a module with no capturing test produces no directory. Wired into the package phase by `InferredMultiProjectAssembler` ahead of `native-image` only when `packaging.properties` sets `native=true`. | this module's `inventory.properties` (`prefix.artifact`) and its siblings' (`prefix.test`, `prefix.nativeimage`) | `native-image/` (the merged config directory) |
...
- `executed` (`TestModule.Run` extends `Java`) accepts `filter`, `tag`, `parallel`, and `reporting` arguments.
  `filter` is a comma-separated list of Java regex entries, each `<classRegex>` or `<classRegex>#<methodName>`;
  `tag` is a comma-separated list of test tags; `parallel` and `reporting` are flags. The **default value**
  of each is resolved from a `jenesis.test.*` system property (`jenesis.test.filter`, `jenesis.test.tag`,
  `jenesis.test.parallel`, `jenesis.test.reporting`): `TestModule`'s public constructor reads them once
  (`System.getProperty` / `Boolean.getBoolean`) to seed the fields. The matching `filter`/`tag`/`parallel`/
  ...
  matched class and method names, the parsed tag set, and the two flags are handed to the engine's
  ...
  `--include-tag=` per tag, the `junit.jupiter.execution.parallel.*` config parameters when parallel, and the
  ...
  scope that produced it left untouched (see *Tag-aware test caching* under [Architecture](#architecture)). The
  transient `force` flag skips that comparison, so the tests execute whatever the record says - what a flaky test
  or a debugging session wants. `reporting` is part of the serialized state (so toggling it
  ...
  `PathPlacement` - the same `ModuleGraph.place` routine `Execute` uses to launch, so a descriptor-bearing or
  `Automatic-Module-Name` jar lands on the module path and a plain jar on the class path. `ModuleGraph`
  accumulates what it places and renders the **relaxations** the resulting graph needs. Unless the launch is a
  **self-contained module graph** - a pure named-module path where every jar is an explicit module and nothing
  ...
  path - it adds `--add-modules ALL-MODULE-PATH,ALL-DEFAULT`. Two things break self-containment: an automatic
  module, which declares no `requires` (so a named module it uses only internally, for example a Spring jar's
  transitive `commons-logging`, is otherwise never resolved), and a non-empty class path, whose unnamed module is
  readable only through modules that are themselves resolved. `ALL-MODULE-PATH` roots what the build put on the
  module path; `ALL-DEFAULT` roots the platform modules nobody named, since a `-m` launch defaults to the initial
  module alone as its root, while code that carries no `requires` of its own - an automatic module, or a
  class-path jar - was written expecting the whole platform to be there, and reads only what resolution
  actually enumerated. Rooting is a launch-only concern - `javac`
  resolves just the `requires` closure the sources actually reference (and in fact forbids the flag when
  compiling a named module), so compilation never needs it.

  Rooting is the only relaxation. Nothing grants a module readability of the unnamed module, so a named module
  cannot use a class-path type at all - `javac` reports *package p is declared in the unnamed module, but module
  m does not read it*, and a launch that got as far as loading the class would fail with `IllegalAccessError`.
  A dependency a module needs to reference is therefore promoted rather than reached: `@jenesis.alias` renames it
  to a file name that derives a module name, which moves it onto the module path where a `requires` can name it.
  That keeps a modular compilation exactly as strict as `javac` makes it, and keeps what compiles here compiling
  for a consumer outside this build - readability of the unnamed module cannot be made transitive, so a granted
  edge would only have hidden the problem until publication. `Javac` and `Javadoc` still split the jars they are
  given between `--module-path` and `--class-path` through the same `PathPlacement`, so a jar that derives no
  module name is never scanned as one.

  A qualified `exports <p> to <m>;` or `opens <p> to <m>;` needs no translation either, including when `<m>` is
  a module alias: an aliased target is the very jar the author meant, renamed, so the module the directive names is
  the module that holds the code (see `@jenesis.alias`). A grant qualified to a module that is not resolved at all
  is left untouched, granting nothing, as before.
...
build pulls in for reference, not just to artifact jars. The modular layouts' `@jenesis.bom` Maven import
deliberately stays outside this channel: its references take no hash and checksum comments inside a fetched BOM
are not read, with reproducibility coming from the pin step pinning the artifacts resolved through the
imported versions. The `<dependencyManagement>` block is the single source
...
compiler it was declared for. `demo/demo-10-annotations` runs a Java annotation processor (Immutables) and
`demo/demo-18-kotlin-plugin` a Kotlin compiler plugin (kotlinx.serialization) this way - each showing that the jar
...
| `jenesis.print.process`          | system property | When `true`, every `ProcessBuildStep` runs in verbose/debug mode: the command line and every line the launched tool writes are echoed to `System.out`, each prefixed with `<command> >>>>` and coloured by origin (a muted grey for the output stream, a muted red for the error stream). The lines are still captured to the `output`/`error` supplement files exactly as before; they are additionally streamed line by line as they are produced. For a forked process the output and error streams are drained concurrently through the build `Executor` so the process never blocks on a full pipe. This is the generic default; a per-command property `jenesis.print.<command>` (e.g. `jenesis.print.javac`, `jenesis.print.checkstyle`, `jenesis.print.native-image`) overrides it for that one command, and the `ProcessBuildStep.verbose(boolean)` wither (inherited by every implementation) overrides both in code. The modules that wrap a forked tool (the language compilers, the code-quality and formatting checks, coverage, mutation, documentation, and `TestModule`) expose the same override as a `printing(boolean)` wither that resolves the per-command property when left unset. Default `false`. |
| `jenesis.print.tests`          | system property | The per-command form of `jenesis.print.process` for the forked test JVM: `-Djenesis.print.tests=true` streams the test process's command and output to the console. `TestModule`'s runner resolves its verbose flag under the `tests` key (rather than the `java` command it actually launches under), so test output can be toggled independently of other `java`-launched steps. The `TestModule.printing(boolean)` wither overrides it in code (leaving it unset resolves the property). Default `false`. |
...
| `jenesis.process.factory` | system property | Selects how the JDK tool steps (`Javac`, `Javadoc`, `Jar`, `JMod`, `JLink`, `JPackage`) launch: `tool` runs them in-process via `ToolProvider` (the default), `fork` runs them as separate `java`-home processes (`bin/javac`, …) - use it under stricter sandboxes that disallow in-process tool runs. Read once by `ProcessHandler.Factory.of()` wherever a step is constructed. Unset defaults to `tool`, except in a GraalVM native image (which has no in-process JDK tools), where it defaults to `fork`. An unknown value fails fast. Under `tool`, runs of `jlink` and `jpackage` are **serialized against each other** by `ProcessHandler.OfTool`: the JDK implements `jlink` with a `static final TaskHelper` holding per-run state, so two in-process invocations corrupt one another (`Resource /java.base/legal/... already present`, `stream has already been operated upon or closed`), and `jpackage` reaches the same state because it builds its runtime through `ToolProvider.findFirst("jlink")`. They are one mutual-exclusion group rather than one lock per tool name, since the collision is between the two of them. Every other tool step - `javac`, `javadoc`, `jar`, `jmod`, `jdeps` - keeps running fully in parallel, each verified to be concurrency-safe. `fork` sidesteps the limitation entirely, and is the way to link several images at once. |
...
| `jenesis.test.force` | system property     | Read by `TestModule` as an implicit default (overridable by the `force(boolean)` wither); when `true`, the test step executes even where no input changed and the recorded scope covers what is asked for - the switch for a flaky test or a debugging session, where the point is to run the tests again rather than to be told they already ran. Like `parallel` it is transient on the step, so forcing neither invalidates the cache nor changes what the next unforced run decides; a run that follows a forced one is compared against the scope that forced run recorded, as after any other execution. With a shared cache configured it does not reach past a cache hit, since the entry is fetched before the step is entered. Default `false` - tests run only when needed. |
...
| `jenesis.test.incremental`    | system property     | Read by `TestModule` when it wires the test step (alongside the existing `jenesis.test.skip` check). When set, and no explicit `filter`/`tag` is in effect, the test step narrows execution to the test classes a change can reach: it builds a class-to-test dependency graph from the compiled bytecode (the test classes plus the code under test, read from `classes/` directories and resolved runtime jars), records a per-class content hash as `test-selection.properties` in its own output, and on the next run diffs that snapshot (via `context.previous()`), takes the changed classes, and emits `--select-class` only for the tests that transitively reference them. The property value names the message-digest algorithm used for that snapshot (e.g. `MD5` or `SHA-256`); an empty value defaults to `MD5`, matching the executor's own change-detection digest. A change that reaches no test runs nothing; a removed class or any non-class change falls back to the full set. It is transient on the step (toggling it neither invalidates the cache nor forces a re-run), and is intended for `jenesis.project.watch` development loops, not as a correctness gate, since static selection cannot observe reflection or other indirect couplings. Default unset (selection off). See *Watching for changes*. |
| `jpackage`  | `packaging.properties`     | A key in the `packaging.properties` file `InferredMultiProjectAssembler` reads from the configuration location (a module's `META-INF/build.jenesis/` folder on the modular layouts, or its `build.jenesis/` folder on `MAVEN`, falling back to the project-wide configuration directory, the project root by default; the first match wins, so a per-module file overrides a project-wide one, selecting bundles per module or for all modules at once). When set, it wires a `jpackage` step in the package phase that runs `jpackage` for every module declaring a main class (a `@jenesis.main` Javadoc tag, or a `<mainClass>` POM property); modules without one are skipped. The value is the `jpackage --type` and must be given explicitly: `jpackage=app-image` (a self-contained launcher plus runtime, the only type that needs no platform-native tooling) or `jpackage=deb|rpm|dmg|pkg|exe|msi` (a native installer). An absent or empty value means no per-module `jpackage` step is wired, so nothing is produced. The `--name`, `--app-version` and the launcher are derived by the `prepare` step from the module's metadata and main class. The `JPackage` step then truncates `--app-version` to its leading dotted-numeric part, dropping a qualifier such as the `-SNAPSHOT` of the default `1-SNAPSHOT` and leaving `1`. `jpackage` parses that option as a dotted version - `CFBundleVersion` on macOS, the MSI `ProductVersion` on Windows - and both reject anything following the digits and dots, while the Linux bundlers accept it, so passing the project's version through unchanged would make packaging succeed or fail depending on the platform. The truncation is therefore applied on **every** platform, so the same value is packaged everywhere and a default-versioned project packages an image without configuring anything. It removes a qualifier and nothing else: a version with no dotted-numeric part at all, `RELEASE` say, is passed through unchanged for `jpackage` itself to reject, since inventing a version number for a project that never stated one would be worse than the error: a module built for the module path (`descriptor.pathPlacement().modular()`, i.e. the `MODULAR`/`MODULAR_TO_MAVEN` layouts) gets a modular launcher (`--module <module>/<class>`, run from `--module-path`) - and unless its staged jars form a self-contained module graph (all explicit named modules), the step adds `--java-options --add-modules=ALL-MODULE-PATH,ALL-DEFAULT` so the generated launcher roots the whole module path and the default platform set. Both an automatic module (declares no `requires`, so a named module it uses only internally, such as a Spring jar's `commons-logging`, is otherwise unresolved) and a plain jar (no descriptor, which becomes a filename-derived automatic module on the module path) break self-containment; jpackage stages everything into one `input/` directory, so there is no separate class path to consider. A classpath module (the `MAVEN` layout) instead gets `--main-jar`/`--main-class` run from `--input`. Each produced image is recorded in `inventory.properties` as `prefix.package`, and the `STAGE` module's `packages` step (always registered, alongside `maven`/`modular`) collects every module's image into `stage/packages/` - the staging analogue of `stage/maven`/`stage/modular`. When no module produced an image, the `packages` stage step simply stages nothing. Application images are staged but never exported (they do not belong in a Maven or module repository). |
| `jmod`     | `packaging.properties`     | A boolean key in `packaging.properties` (located as described for `jpackage`). When `true`, it wires a per-module `jmod` step that runs the `jmod` tool over every modular module's `classes/`, producing `jmods/<module>.jmod` (modules with no `module-info.class` are skipped). `Inventory` records it as `prefix.jmod`, and `ModularStaging` hardlinks it **alongside the jar** as `<moduleName>/[<version>/]<moduleName>.jmod` in `stage/modular`. A `.jmod` is the link-time form of the module (consumable by `javac`/`jlink`, never at runtime), which is why it is staged into the module repository next to the jar rather than into `artifacts/`. Default `false`. |
| `jlink`    | `packaging.properties`     | A boolean key in `packaging.properties` (located as described for `jpackage`). When `true`, it wires a `jlink` step in the package phase that runs the `jlink` tool over every modular module plus its runtime dependency jars, producing a custom runtime image in `runtime/`. The root module (`--add-modules <module>`) is supplied by the `prepare` step from the module's name, so non-modular modules (which get no `--add-modules`) are skipped. `Inventory` records the image as `prefix.image`, and the `STAGE` module's `runtime` step (`ImageStaging`) collects every module's image into `stage/runtime/`. Like the runtime image embedded by `jpackage`, it is staged but not exported. When `jmod=true` is also set, `jlink` links from the produced `.jmod` (the richer link-time form, which can carry native libraries `jlink` places into the image) instead of the jar - the two never both feed `jlink`, which would be a duplicate module. Default `false`. |
| `modules.properties`  | build configuration file   | A build-configuration file (located as described for `jpackage`'s `packaging.properties`, so **per module**: a module's `META-INF/build.jenesis/` or `build.jenesis/` folder first, then the project-wide configuration directory) that makes a dependency closure **modular**. It is a file of its own rather than a `packaging.properties` key because it is not a packaging concern: it changes what the module compiles and tests against, and only incidentally what is linked. Its single `mode` key takes `declared`, `synthetic` or `none` and defaults to `declared`, so an empty file is the strict rewrite - the reading `ModularizeModule.configured(Path)` performs, the same shape as `Sbom.configured(Path)` and every other config-file-discovered plug-in. `none` performs no rewrite, which is also what an absent file means - the default, so nothing about a build changes until the file exists. Because the file is located per module and the **first** match wins, `mode=none` is how a single module opts out of a project-wide file that would otherwise rewrite it. <br><br>Under `declared` or `synthetic` a `ModularizeModule` runs at the head of the module's assembly and translates the resolved closure into an **equivalent closure in which every jar is an explicit named module**, in three steps. Its `prepare` step reads `dependencies.properties` and splits the closure into the two folders the `JDeps` step consumes: a jar that already declares an explicit `module-info` is hard-linked into `modules/<module>.jar` and otherwise left alone, every other jar is hard-linked into `analyzed/<module>.jar` under the name it is to carry - its `Automatic-Module-Name`, or the name a `@jenesis.alias` gave it, both read through `PathPlacement.moduleDescriptor`, so an alias's `.module` sidecar counts and a name merely derivable from a file name deliberately does not. A jar that declares neither has no stable name of its own, which is what the mode decides: `declared` fails the build naming the coordinate and the three ways out, `synthetic` names it `build.jenesis.pseudo.module<hash>`, where `<hash>` is the leading 128 bits of the jar's SHA-256 digest in hex - so `sha256sum` on the jar reproduces it. The literal `module` in front of the digest is not decoration: a module name segment must be a Java identifier, and a hex digest usually starts with a digit, which `jlink` rejects outright with `Invalid module name: '3f5a9c' is not a Java identifier`. Two jars claiming one module name fail the build, and so does a **split package**: the `modularize` step builds a package-to-module map over the whole staged set plus the system modules, and a package carried by two of them stops the build naming the package and both modules. That check is not decoration - a split package is harmless on the class path, where the first jar wins, but makes the closure unresolvable once every jar is a module, and the failure is otherwise badly placed: `javac` only reports it if the project happens to read both modules, and `jlink` links the image without complaint, leaving a runtime that dies on startup with `LayerInstantiationException`. The step also writes the `staged.properties` that carries the naming decision to the last step - a file private to the module, and the only thing the two steps exchange - and the `process/jdeps.properties` that drives the `JDeps` step (`--multi-release <feature>` and `--ignore-missing-deps`, this module's policy rather than the tool step's). Its `describe` step is the shipped `JDeps` step. Its `modularize` step then writes exactly `modular/` and `modular.properties` - the module jars and an index over them, and nothing else. It deliberately does **not** write a `dependencies.properties`: that name is what every provenance consumer scans for, so by not claiming it the rewrite stays invisible to `Inventory`, `Sbom`, `LicenseCheck`, `Bom` and `Versions`, which keep reading the resolved closure and need no knowledge of this feature at all. What routes the path consumers instead is one rule in `Dependencies.select` / `Dependencies.all`: a folder offering a `modular.properties` is read through it in preference to its `dependencies.properties`. That single rule reaches `javac`, `java`, `javadoc`, `jlink`, `jpackage`, `bundle`, `launcher`, `docker`, `native-image`, the test compile and run, and every tool module, none of which changes. A jar staged under `modules/` is hard-linked into `modular/` **byte for byte**, while a jar staged under `analyzed/` gets a `module-info.class` synthesized and injected - its name, package set and `META-INF/services` entries from the JDK's own automatic-module derivation (`ModuleFinder`), its `requires` from the `module-info.java` that `jdeps` generated (parsed with `JavacTask`, so the `transitive` and `static` modifiers survive), and its `uses` from the `ServiceLoader.load` call sites found in its byte code (`ClassFile`), filtered against the package-to-module map built from the whole staged set plus the system modules. The descriptor is emitted with the `ClassFile` API at class file version 53.0 as an **open** module exporting every package it contains - as close as a named module gets to how the jar behaves on the class path, where nothing is encapsulated - and a provider whose package is not in the jar is dropped, since a descriptor may only provide what it contains. Because the bytes change, any signature is stripped (`META-INF/*.SF`, `*.DSA`, `*.RSA`, `*.EC`, `SIG-*` and the per-entry digests in the manifest): a jar whose content no longer matches its signature fails to load. Only `JDeps` is a public step; `prepare` and `modularize` are private to `ModularizeModule`, which is the whole unit a build wires - and the module resolves only its last step onto its own name (`resolve` returning `Optional.of("")` for it and `Optional.empty()` for the rest, the shape `BuildStep.asModule` uses), so a consumer depends on the module by name and never on a step path inside it. The staging folders never reach a consumer either. <br><br>That rewritten closure then *replaces* the resolved one for everything the module builds - `javac`, the byte-code validators, `javadoc`, the test compile and the test run, and every package-phase step (`jlink`, `jpackage`, `bundle`, `launcher`, `docker`, `native-image`). Replacing rather than adding is the point: a broken module graph - a split package, an unnamed jar, a `requires` nothing provides - then fails at compile or test time rather than first in the production image. See the [`module-alias`](demo/demo-31-module-alias/README.md) demo, whose alias is exactly the case that needs it. Everything that describes what was *fetched* keeps reading the resolved closure and is untouched by construction: `sbom` and `compliance` (whose components and license and vulnerability findings are about the upstream artifacts) and `Inventory` - which is what feeds `pin`, so a rewritten jar's bytes can never reach a `@jenesis.pin` checksum. The modular index carries no checksums at all, since nothing reads one from it. The build phase points its consumers at the rewrite by replacing `descriptor.artifacts()` with the module's own key, and the package phase drops exactly the folders `descriptor.artifacts()` names (normalising the `../` nesting on both sides, since the same step is addressed from different depths) - both by name, neither by inspecting a folder's contents. The rewrite is skipped outright under `PathPlacement.MODULE_PATH`, where it would be a no-op: the `MODULAR` layout resolves by module name through `ModularJarResolver`, so every jar in that closure is already an explicit module and the rewrite would only copy. It is *not* skipped under `CLASS_PATH`, because that is the `MAVEN` layout, where a project that carries a `module-info.java` is compiled as a module all the same - `PathPlacement.CLASS_PATH.forModuleInfo(true)` promotes it to `INFERRED` - so a modular application built with a POM benefits from the rewrite exactly as a modular layout does. One consequence is worth knowing: a fully explicit closure is a **self-contained** module graph, so `ModuleGraph` stops adding `--add-modules=ALL-MODULE-PATH,ALL-DEFAULT` to the launchers `bundle`, `launcher`, `jpackage` and `native-image` generate. That is the point - the graph now resolves from the main module's own `requires` - but a module reached only reflectively, which the blanket rooting used to drag in, has to be named explicitly from then on. |
| `jenesis.jreleaser.dryRun` | system property | Default `true`. The `release/jreleaser/execute` step passes `--dry-run` to JReleaser, which performs every local phase and skips every remote one. Set it to `false` to publish; it is the single switch that separates a rehearsal from a release, and the reason no selector alone can publish. |
| `jenesis.jreleaser.command` | system property | The JReleaser command to run, default `full-release`. Use `deploy`, `release`, `announce` and so on for finer-grained retries after a partly finished release. |
| `jenesis.jreleaser.executable` | system property | The JReleaser binary, default `jreleaser`, located through `GRAALVM_HOME`, then `java.home/bin`, then `PATH`, as every non-JDK tool step is. |
| `jenesis.jreleaser.config` | system property | An explicit JReleaser configuration file, resolved against the project root, overriding the `jreleaser.yml`/`.yaml`/`.toml`/`.json` lookup. A value naming a file that does not exist is an error rather than a silent skip. |
| `jenesis.print.jreleaser` | system property | The per-command form of `jenesis.print.process`, defaulting to `true` for this step alone so a multi-minute upload is not silent. |
| `bundle`   | `packaging.properties`     | A boolean key in `packaging.properties` (located as described for `jpackage`). When `true`, it wires a `bundle` step in the package phase that writes `bundle/bundle.zip` for every module declaring a main class (a `@jenesis.main` Javadoc tag, or a `<mainClass>` POM property); modules without one are skipped. The zip carries every jar the module's launcher needs, split into `classpath/` and `modulepath/` folders (the modular split mirrors `Execute`: real and automatic modules go on the module path, the rest on the class path; a non-modular launcher puts everything on the class path), plus an `application.properties` with a `mainClass` property and, for a modular launcher, a `mainModule` property (plus a `javaOptions` entry with the JVM options the graph needs - `--add-modules=ALL-MODULE-PATH,ALL-DEFAULT` when an automatic module or class-path jar is present - written only when it needs any). It is meant as a single, self-contained input for building a container image or other deployment, the lightweight counterpart to `jpackage` (which invokes the `jpackage` tool). Default `false`. |
| `launcher`  | `packaging.properties`     | A boolean key in `packaging.properties` (located as described for `jpackage`). When `true`, it wires a `launcher` module (`LauncherModule`) in the package phase for every module declaring a main class. The module resolves the published `build.jenesis:build.jenesis.launcher` artifact (floating `RELEASE`, in its own `launcher` group) from the configured repositories, then a `Launcher` step produces a **single executable jar** (`launcher/<name>.jar`): the launcher's `build/jenesis/launcher/*.class` are shaded into the jar root as the `Main-Class` (`build.jenesis.launcher.Launcher`, its own `module-info`/manifest dropped), each dependency is exploded into its own `classpath/<jar>/` or `modulepath/<jar>/` subfolder (the same modular split as `bundle`/`Execute`), and `application.properties` carries `mainClass`, `mainModule`, and the `classpath` order. At run time the launcher reconstructs the module graph from those subfolders, so `java -jar foo.jar` keeps full modularity without flattening into a fat jar. The counterpart to `bundle` that needs no launch script. Default `false`. |
| `native`   | `packaging.properties`     | A boolean key in `packaging.properties` (located as described for `jpackage`). When `true`, it wires a `native-image` step into the **package phase** (the cross-module level that runs after every module's build) for every module declaring a main class (a `@jenesis.main` Javadoc tag, or a `<mainClass>` POM property), ahead-of-time compiling the launcher and its closure into a single native executable; modules without a main class are skipped. The launcher is read from the neutral `launcher.properties` descriptor the `prepare` step writes (`mainModule`/`mainClass` for a modular launcher, otherwise `mainClass` with the jars on the class path) - not `jpackage`'s own command file. The closure is split the same way the run does (descriptor-bearing jars onto `--module-path`, plain jars onto `-cp`); unless that is a self-contained module graph (a pure named-module path), it additionally gets `--add-modules ALL-MODULE-PATH,ALL-DEFAULT` - the same rooting fix the run uses, triggered by an automatic module or a class-path jar. The reachability metadata captured by the module's test (enabled by a `graal.properties` file, see below) is collected from the sibling inventories by a `reachability` step (`NativeImageMetadata`, routing each capture by the `prefix.test`/`prefix.artifact` pair) and passed as `-H:ConfigurationFileDirectories`, so the image builds in one pass with no committed `META-INF/native-image/`. Each produced executable is recorded in `inventory.properties` as `prefix.native`, and the `STAGE` module's `native` step (`ImageStaging`, always registered alongside `packages`/`runtime`) collects every module's binary into `stage/native/` - the native-image analogue of `stage/packages` (jpackage) and `stage/runtime` (jlink). Like those images it is staged but never exported (a native binary does not belong in a Maven or module repository). Unlike the JDK tool steps, `native-image` is always an external process located via `GRAALVM_HOME`, then `java.home`, then `PATH`, so a GraalVM that ships `native-image` must be installed. Default `false`. |
| `docker`   | `packaging.properties`     | A valued key in `packaging.properties` (located as described for `jpackage`). Its value is the **base image** the generated `Dockerfile` starts `FROM`, and it must be given explicitly - `docker=eclipse-temurin:25-jre` - because the base is the one thing the build cannot infer; an absent or empty value means no `docker` step is wired, so nothing is produced. When set, it wires a `docker` step in the package phase that writes a container build context (`docker/Dockerfile` plus the `classpath/`/`modulepath/` jars it copies) for every module declaring a main class (a `@jenesis.main` Javadoc tag, or a `<mainClass>` POM property); modules without one are skipped. Everything else is a fixed default: the app is copied under `/app` and started by an exec-form `ENTRYPOINT`. There is deliberately no second configuration file, because a Dockerfile inherits `ENV`, `WORKDIR`, `USER` and `EXPOSE` from its base image, so anything else you want in the image belongs in a base image you control rather than in build configuration. The build itself never runs a container tool, so no Docker installation is required to produce the file. Nothing in the emitted file is Docker-specific either: it is plain OCI builder syntax, so `podman build` and `buildah bud` consume the same staged folder unchanged (Podman resolves a `Containerfile` first and falls back to the `Dockerfile` written here, which is why the file keeps that name - it is the one name both builders find without a `-f` flag). `Inventory` records the context as `prefix.docker`, and the `STAGE` module's `docker` step (`ImageStaging`) collects it into `stage/docker`, so `docker build target/stage/docker/output` is the whole remaining step. Each module's own context is always complete under its `docker/` output; staging merges them into one folder the way `stage/runtime` merges `jlink` images, so a project packaging several main classes at once should build from the per-module output (or stage one module at a time) rather than from the merged folder. |
| `jenesis.sbom.cyclonedx` | system property | Read by `InferredMultiProjectAssembler`. A CycloneDX software bill of materials is emitted **by default**: the assembler wires a per-module `sbom` step (before the jar is sealed) that emits it in-process from the module's resolved dependency graph, content hashes, and captured licenses (already normalized to SPDX identifiers at extraction, so each is emitted as a CycloneDX `license.id` when the identifier is recognized, otherwise as a `license.name`). This property is a default-`true` **suppress** override (a boolean): set `-Djenesis.sbom.cyclonedx=false` to skip the SBOM entirely, regardless of any file. The format is chosen by an optional `sbom.properties` in the configuration directory: `format=json` (the default, also used when the file or the `format` key is absent) or `format=xml` selects the format, `format=none` disables the SBOM, and any other value fails the build with an `IllegalArgumentException`. The document is placed three ways: **embedded** in the jar at `META-INF/sbom/<artifact>.<ext>` (with `Sbom-Format`/`Sbom-Location` `MANIFEST.MF` headers and a `META-INF/NOTICE`); recorded in `inventory.properties` as `prefix.report.sbom` and collected by `ReportStaging` into `stage/reports/sbom/<module>/`; and, when a Maven repository is staged (`MAVEN`/`MODULAR_TO_MAVEN`), attached as `<artifact>-<version>-cyclonedx.<ext>` next to the pom and jar so `export` publishes it to Maven Central. The document describes the project itself as its `metadata.component` from the captured POM metadata (its declared license, its developers as CycloneDX `authors`, its `description`, and its homepage/SCM as `website`/`vcs` external references), filling in only what is declared: an unset version (the `1-SNAPSHOT` placeholder) is omitted rather than fabricated. It also carries a `serialNumber` (`urn:uuid:...`) derived deterministically from the document content (a name-based UUID, no wall-clock `timestamp`), so the SBOM stays byte-for-byte reproducible. Default `true`. |
...
| `jenesis.format.java`, `jenesis.format.rewrite`, `jenesis.format.<tool>` | system property | Read by `InferredSourceFormattingModule` (wired as the per-module `format` step). A Java formatter is selected by a `javaformat.properties` file (`formatter=google\|palantir`), the same way ktlint / scalafmt activate from `.editorconfig` / `.scalafmt.conf`; `jenesis.format.java` / `jenesis.format.ktlint` / `jenesis.format.scalafmt` (each default `true`) switch the respective formatter off. `jenesis.format.rewrite` (default `false`) flips the whole chain from verify mode (a CI gate that fails on an unformatted source without writing it) to rewriting sources in place (it sets `.verify(false)` on each formatter). Default: no Java formatter unless a `javaformat.properties` file is present, ktlint/scalafmt on, verify mode. |
| `jenesis.observe.jacoco` | system property | Read by `InferredTestObservationModule` (wired as the per-module `observed` step that wraps the test run). JaCoCo is enabled by the presence of a `jacoco.properties` file in the configuration directory (the project root by default), which adds JaCoCo as a coverage agent that `-javaagent`-attaches to the test JVM and writes `jacoco.exec`, which the `JaCoCoModule` then renders into an HTML/XML report under `reports/jacoco/`. This property is a default-`true` **suppress** override: set it to `false` to switch JaCoCo off even when the file is present (the test run is then unwrapped). The `.jacoco(boolean)` wither sets it directly in code. Default `true`. |
| `jenesis.observe.native` | system property | Read by `InferredTestObservationModule`, the GraalVM counterpart of `jenesis.observe.jacoco`. The tracing agent is enabled by the presence of a `graal.properties` file in the configuration directory, which adds the `NativeImageAgent` engine, `-agentlib:native-image-agent=config-output-dir=...`-attaching the GraalVM tracing agent to the test JVM so it records the reflection, JNI, resource, proxy and serialization the tests exercise. This property is a default-`true` **suppress** override: set it to `false` to switch the agent off even when the file is present. The agent ships in the GraalVM runtime (no coordinate to resolve), so the build must run on a GraalVM JDK (or with `GRAALVM_HOME` set, as the test JVM inherits the build's `java.home`); on a stock JDK the test run fails to load the agent. `NativeImageAgentModule` then stages the captured config (`reachability-metadata.json` and friends) under `nativeimage/` - a build-internal location, deliberately not a `reports/` report, since the metadata is fed back into the image build (by the package phase's `NativeImageMetadata` router, recorded in the inventory as `prefix.nativeimage`) rather than read by an external tool. The same config can also be committed into `META-INF/native-image/` where `native-image` (the `native` key in `packaging.properties`) auto-discovers it. The `.nativeImage(boolean)` wither sets it directly in code. Default `true`. |
| `jenesis.mutate.pitest` | system property | Read by `InferredTestObservationModule` (wired as the per-module `mutate` step). PiTest mutation testing is enabled by the presence of a `pitest.properties` file in the module (the same config-file-discovery convention as the linters), which resolves PIT and its JUnit 5 plugin in a `pitest` group, seeds faults into the compiled classes, re-runs the tests against each mutant via `PiTestModule`, and reports under `reports/pitest/` which mutants survived. This property is a default-`true` **suppress** override: set it to `false` to keep the `pitest.properties` file in place but skip the mutation run. The `.pitest(boolean)` wither sets it directly in code. Default `true`. |
| `licensing.properties` | configuration file | Read by `InferredComplianceModule` (wired by `InferredMultiProjectAssembler` as the per-module `compliance` module). License compliance is a convention: it is enabled purely by the presence of a `licensing.properties` file in the configuration location (resolved through the same `configurations(...)` search as the other config files, so profile folders and per-module folders are honoured), which drives a `LicenseCheck` over the resolved dependency licenses: `allowed` / `denied` are comma-separated lists of license names or category keywords (e.g. `permissive`), `unknown=ignore\|warn\|fail` (default `fail`) sets the policy for a dependency whose license cannot be determined, and `override.<coordinate>=<license>` records a license for a dependency that declares none. An unrecognized key fails the build. To skip the check, omit the file (or set `jenesis.compliance=false` to skip both compliance checks at once). |
| `vulnerability.properties` | configuration file | Read by `InferredComplianceModule`, the vulnerability counterpart, and enabled the same way: purely by the presence of a `vulnerability.properties` file in the configuration location. It wires an `OsvDownload` step (querying the OSV database; `osv.endpoint=<uri>` overrides the API endpoint) feeding a `VulnerabilityCheck` over the resolved dependencies: `severity=low\|medium\|high\|critical` sets the threshold at or above which a known advisory fails the build, and `warn=true` (default `false`) reports findings without failing. An unrecognized key fails the build. To skip the check, omit the file (or set `jenesis.compliance=false`). |
| `jenesis.compliance` | system property | A default-`true` **suppress** override for the whole `InferredComplianceModule`: set it to `false` to keep the `licensing.properties` / `vulnerability.properties` files in place but skip both the license and the vulnerability check, in the same manner as the other inferred plugins (`jenesis.source.checkstyle`, `jenesis.validator.spotbugs`, and so on). The `.enabled(boolean)` wither sets it directly in code. Default `true`. |
...
| `jenesis.resolver.maven` | system property | The strategy the Maven resolver negotiates versions with - `maven`, `latest`, `release` or `closest` - naming one of the four `MavenDefaultVersionNegotiator` factories. Unset is not a fourth behaviour: it constructs the plain `MavenPomResolver`, whose own default is `maven()`, so `maven` only gives that default a name. `maven` follows Maven: a coordinate resolves to its declared version, `RELEASE`/`LATEST`/a range are looked up in the artifact's `maven-metadata.xml`, and when a coordinate is requested at several versions the one declared nearest the root wins - *unless* one of the competing requests is a range, in which case every range counts as a hard requirement and the highest published version satisfying all of them at once is taken (a version nobody declared, if that is what the ranges intersect at; none at all fails the build naming the coordinate). `closest` is that minus the range arbitration: the nearest declaration stands and a competing range never gets to argue, so no metadata is fetched to settle a conflict. `latest` and `release` replace the *per-declaration* half instead: the declared version is not read at all and every coordinate resolves to the `<latest>` or `<release>` entry of its `maven-metadata.xml`, which means a metadata fetch per coordinate rather than per floating one - a coordinate the repository publishes no metadata for, such as a sibling module of this very build, fails the resolution outright - and that pinned versions are overridden - the checksum pinned alongside is keyed by the version it was pinned for, so it stops matching and is dropped, and under `-Djenesis.dependency.pin=strict` the build then fails. Treat those two as upgrade probes, not as build modes. The property is read by `MavenPomResolver`'s no-argument constructor, which maps the name straight to the matching factory, so every plainly constructed resolver honours it - the one the `MAVEN` and `MODULAR_TO_MAVEN` layouts wire included - and nothing has to thread the choice down from `Project`. In code, the strategy is chosen by constructing the resolver with it (`new MavenPomResolver(MavenDefaultVersionNegotiator.latest())`) and handing that to `Project.resolvers(...)`, which the layouts leave untouched; the constructor declares the `<S extends Supplier<MavenVersionNegotiator> & Serializable>` bound, so a lambda returning a custom `MavenVersionNegotiator` is emitted serializable, as the step-config hash requires. The resolver asks its supplier for a negotiator once per resolution, which is what gives each resolution its own metadata cache. An unrecognised property value fails the resolver's construction, naming the property and the four names. |
| `jenesis.resolver.module` | system property | The module-path counterpart of `jenesis.resolver.maven` - `first`, `ignore` or `fail`, naming one of the three `ModuleVersionNegotiator` factories, with unset constructing the plain `ModularJarResolver` whose own default is `first()`. It decides what a modular resolution does with the compiled versions a `module-info.class` records for its own requirements (`ModuleDescriptor.Requires.rawCompiledVersion()`), which rank behind a `@jenesis.pin` and ahead of the inline `<module>/<version>` spelling of a root coordinate (only a root can carry one - a transitive requirement enters the walk as a bare module name). `first` keeps the version the descriptor nearest the roots recorded (the walk is breadth-first, requirements visited in name order) and drops every later disagreement without a word. `ignore` keeps none, so a module resolves by its pin, else its inline version, else as whatever the repository serves for the bare name - the modular analogue of `latest`. `fail` keeps the first like `first`, but two descriptors recording *different* versions for the same module fail the build with a message naming both versions and both declaring modules, which is the only way that disagreement becomes visible at all. The unsafe-version check (empty, `..`, a slash, a backslash, or a leading colon) runs ahead of the negotiator and therefore fires under every mode, and a module whose version is pinned is left to its pin under every mode - the negotiator is never consulted for it. The property is read by `ModularJarResolver`'s constructors that take no negotiator, exactly as `jenesis.resolver.maven` is read by `MavenPomResolver`, so the resolver the `MODULAR` layout wires honours it without `Project` knowing about it. In code, the mode is chosen by constructing the resolver with it (`new ModularJarResolver(false, null, ModuleVersionNegotiator.fail())`) and handing that to `Project.resolvers(...)`; that constructor declares the same serializable bound, and the resolver asks its supplier for a negotiator once per resolution - the same shape as the Maven side, so a stateful negotiator is possible here too even though none of the three modes needs one. An unrecognised property value fails the resolver's construction, naming the property and the three names. |
...
| `jenesis.project.root`          | system property | Overrides the project root that `Project` scans for `module-info.java` / `pom.xml` (default `.`). Command-line only: the root locates `jenesis.properties` itself, so setting it inside any loaded properties file fails the load with an `IllegalStateException` rather than half-applying. |
| `jenesis.project.properties`    | system property | A comma-separated list of profile names that `Project.loadJenesisProperties` loads; each name `<name>` resolves to a `jenesis-<name>.properties` file (a `.properties` suffix on the name is ignored), relative to the folder of the file that declares them. Any loaded file (a `-D`, the project `jenesis.properties`, the user-global `jenesis.properties`, or another profile) may set `jenesis.project.properties` to chain further profiles transitively until all are loaded; a profile whose `jenesis-<name>.properties` file does not exist is skipped rather than an error, since a profile may exist only as a `<name>/` configuration folder (see `jenesis.project.configuration`). Everything merges with `putIfAbsent` in four precedence tiers, highest first: **explicit `-D`** > the **profiles** > the project `jenesis.properties` > the user-global `jenesis.properties`. So profiles override the project's base file (note: this differs from the pre-existing behaviour, where the base file won over profiles), and the project file overrides the user-global one. See *Configuration* above and the [`profiles`](demo/demo-15-profiles/README.md) demo. |
| `jenesis.project.global`        | system property | Names the folder holding the user-global `jenesis.properties`, read at `<folder>/.jenesis/jenesis.properties` as the lowest-precedence layer (shared defaults across every project, beaten by the project file, the profiles, and `-D`). Default is the JVM's `user.home`, so `~/.jenesis/jenesis.properties` is read when present; a missing file or folder is silently ignored, like the base `jenesis.properties`. Set it to an empty string (`-Djenesis.project.global=`) to disable user-global loading entirely. It is read only from a `-D` flag or the project `jenesis.properties` (it has to be known before the user-global file is loaded); setting it in a profile or in the user-global file itself fails the load with an `IllegalStateException` rather than being silently ignored. The user-global file may itself declare profiles via `jenesis.project.properties`, resolved relative to its own `.jenesis` folder and merged into the profile tier. |
...
| `spdx.properties`     | configuration file | An optional file in the configuration location that extends the built-in license normalization/classification tables the `Dependencies` step applies at extraction. It is resolved through the same `configurations(...)` search as the other config files (so profile folders and per-module folders are honoured) and bound alongside the metadata overrides to reach the `Dependencies` step. Each file uses one prefixed key space: `alias/<declared name>=<SPDX id>` adds a name alias, `category/<SPDX id>=<category>` adds a classification. The `spdx.properties` name is deliberately distinct from the compliance `licensing.properties` (the enforcement policy, a separate concern). Supplying it is rarely necessary (built-in tables only by default). |
| `bom.properties`     | configuration file | Read by `Project`'s `BomAwareAssembler`, which the `MODULAR` and `MODULAR_TO_MAVEN` layouts wrap around the assembler (the `MAVEN` layout does not, so the feature is unavailable there). Its presence in the configuration location (resolved through the same `configurations(...)` search as the other config files, so per-module `META-INF/build.jenesis/` and profile folders are honoured) switches the layout to *emit* a bill of materials of the module's own resolved dependencies. A `Bom` step reads the resolved `dependencies.properties` (the `main` group, `compile`+`runtime`) and `module.properties`, then writes `pin-<module>.properties` into its `bom/` output folder: the module's full closure, keyed group-less in the pin token grammar (a bare `<module>`, `<groupId>/<artifactId>`, or a verbatim `<repository>/<coordinate>` - the exact inverse of `Dependencies.bomEntries`, so it round-trips) with a version and checksum per coordinate. `Inventory` collects it under `prefix.bomfile` (deliberately **not** in the module's `runtime`/module-path closure, since it is a properties file), and `ModularStaging` hardlinks it into the module repository as `<module>/[<version>/]<module>.properties` - exactly the `:properties` type a consuming `@jenesis.bom <module>` resolves, so another project imports the published closure the same way it imports a hand-written BOM. It is published **only** into the Jenesis repository: `MavenRepositoryStaging` is untouched, so the BOM never appears in the Maven `export`. Presence alone activates emission (the file is otherwise empty and reserved for future options), mirroring how `packaging.properties` gates `jpackage`. See the [`bom`](demo/demo-28-bom/README.md) demo, which both consumes and publishes a BOM. |
| `jenesis.project.configuration` | system property | Project-wide directories (each relative to the project root) the inferred quality and formatting tools, plus the `packaging.properties` selector and the `licensing.properties` / `vulnerability.properties` / `spdx.properties` compliance and license files, search for their configuration files (`checkstyle.xml`, `pmd.xml`, `spotbugs-exclude.xml`, `.editorconfig`, `.scalafmt.conf`, and so on); a path-separated list, defaulting to `build.jenesis/` under the project root. The bare root is deliberately not searched by default, so a conventionally named file dropped at the root (say an `.editorconfig` an editor creates) cannot silently alter the build; add `.` to the list to opt into root-level files. Each module is also searched first in its own per-module folder (`build.jenesis/` next to its `pom.xml` in the MAVEN layout, `META-INF/build.jenesis/` under its sources in the modular layouts), so per-module configuration overrides these project-wide ones. In the MAVEN layout each pom builds a main and a test module, and each additionally searches a scope-specific folder before the shared per-pom `build.jenesis/`: the main module reads `src/main/build.jenesis/` and the test module `src/test/build.jenesis/` (`MavenModuleDescriptor.configurations()` supplies the implicit list) - so e.g. coverage (`jacoco.properties`) can be switched on for one module's tests alone, or a checkstyle rule set can apply to main sources without also gating the test sources. The modular layouts need no such split, since their test modules are separate modules with their own `META-INF/build.jenesis/`. Set to an empty string to drop the project-wide directories and rely only on per-module configuration. An entry may be a **reference** instead of a path: a bare `@` splices in the default (`build.jenesis/` under the project root) at that position, so `-Djenesis.project.configuration=../shared:@` adds a shared folder without dropping the default, and `@<name>` splices the value of the system property `<name>` (env fallback `<name>`), parsed with the same grammar; unresolved or circular references fail. The same reference syntax applies to `jenesis.project.boms`, where a bare `@` splices in the resolved configuration locations, making "the configuration folders plus one more" expressible. Held on `Project` as a `SequencedSet<Path>` (settable in code via the `configuration(Path...)` wither, the empty set meaning none). Read by the `new Project()` constructor; the searched folders are made absolute and de-duplicated, and any folder that does not exist is ignored. |
...
| `jenesis.repository.retries`    | system property | How many times a failed repository fetch is retried (default `2`, so three attempts in total; `0` disables retrying). A retry happens for transient failures only: an HTTP `429` or any `5xx` response, and a connection that fails or drops (connect errors, resets, timeouts). A `4xx` such as `404` stays an immediate miss and a retry-exhausted failure stays a hard error, so a repository outage is reported rather than silently rerouted. A `Retry-After` header on a `429`/`5xx` response is honored, capped at 30 seconds so a hostile server cannot stall the build. Read as the default of `Repository.Retry.of()`; code overrides per repository via the `retry(Repository.Retry)` wither on `MavenDefaultRepository`, `JenesisModuleRepository` and `JenesisRawGitRepository`, with `Retry.retries(int)` / `Retry.backoff(Duration)` withers on the value itself. |
| `jenesis.repository.backoff`    | system property | The initial wait in milliseconds before the first retry (default `125`), doubling with each further attempt - the same shape as Gradle's dependency-download backoff. Overridable in code through the same `Repository.Retry` withers as `jenesis.repository.retries`. |
| `jenesis.maven.uri` | system property (env fallback `MAVEN_REPOSITORY_URI`)| Overrides the default `MavenDefaultRepository.of()` upstream URL (`https://repo1.maven.org/maven2/`). Useful for pointing at an internal mirror; a trailing slash is added automatically if missing. The value may be a comma-separated list of URLs: the factory chains one repository per entry via `prepend`, so entries are queried left to right and the first hit wins; checksum sidecars are validated against the entry that is being queried. Each entry may chain `\|`-separated group ids after its URL - `<url>\|<group>\|...` wraps that entry in `MavenRepository.filter`, so it only serves a group id that equals one of the arguments or sits below it at a dot boundary (`com.corp` matches `com.corp` and `com.corp.core`, not `com.corporate`); every other request is a miss that falls through to the next entry. `https://nexus.corp/maven\|com.corp,https://repo1.maven.org/maven2/` thus routes `com.corp.*` to the mirror and everything else to Maven Central. An entry may also be a **reference** instead of a URL: a bare `@` splices in the default configuration at that position (the `MAVEN_REPOSITORY_URI` value if set, else the built-in default; inside the environment value itself, `@` means the built-in default), and `@<name>` splices the value of the system property `<name>`, falling back to the environment variable `<name>` - the spliced text is parsed with the same grammar, so it may itself be a list with filters and further references. `https://nexus.corp/maven,@` thus tries the mirror first and falls back to whatever the default would have been, without restating it. Filter arguments on a reference (`@corp.mirrors\|com.corp`) wrap the entire spliced chain. An unresolved or circular reference fails the build with an `IllegalStateException`. A non-HTTPS scheme (e.g. `http://`) is rejected unless `-Djenesis.repository.insecure=true` is set.                                       |
...
| `jenesis.module.uri` | system property (env fallback `JENESIS_REPOSITORY_URI`)| Overrides the upstream base URL that the static factory `JenesisModuleRepository.of(JenesisRepository.Scope)` points at (default `https://repo.jenesis.build/`, the public overlay; see *Jenesis Repository layout for Java modules*). A trailing slash is added automatically if missing; the factory then appends `module/` (for `Scope.MODULE`) or `artifact/` (for `Scope.ARTIFACT`) to form the actual repository root. The value may be a comma-separated list of base URLs: the factory chains one repository per entry via `prepend`, so entries are queried left to right and the first hit wins (the subpath is appended to every entry). Each entry may chain `\|`-separated module ids after its URL - `<url>\|<module>\|...` wraps that entry in `JenesisRepository.filter`, so it only serves a module id (classifier and version stripped) that equals one of the arguments or sits below it at a dot boundary; every other request is a miss that falls through to the next entry. Entries may also be references, exactly as for `jenesis.maven.uri`: a bare `@` splices the default configuration (the `JENESIS_REPOSITORY_URI` value, else the public overlay), `@<name>` the value of the system property or environment variable `<name>`, recursively and with unresolved or circular references failing the build. Useful for pointing at a mirror or a privately hosted publication of the same on-disk shape. The explicit `URI`-arg constructors bypass this variable entirely (and bypass the subpath append - the caller passes the full root). |
| `jenesis.module.local` | system property (env fallback `JENESIS_REPOSITORY_LOCAL`)| Overrides the local Jenesis module repository directory (default `~/.jenesis`) used by `JenesisModuleRepositoryExport` (writes the staged module tree into it; directory created on demand) and by the static factory `JenesisModuleRepository.ofLocal()` (reads module jars back from it). `JenesisModuleRepository.of(...)` puts that same local repository in front of the remote overlay, so it honours this variable too; the overlay itself is governed by `JENESIS_REPOSITORY_URI` / `JENESIS_REPOSITORY_TOKEN` instead. |
| `jenesis.module.token` | system property (env fallback `JENESIS_REPOSITORY_TOKEN`)| When set, repositories created by `JenesisModuleRepository.of(...)` send the value verbatim as an `Authorization` header on every HTTP fetch (e.g. `Bearer <token>` or `Basic <base64(user:pass)>`). Ignored for `file://` roots and any non-HTTP scheme. As with `MAVEN_REPOSITORY_TOKEN`, the header is sent only to the configured origin and is dropped when a fetch is redirected to a different scheme, host, or port - so the public overlay's 302 to Maven Central (a different host) is followed without forwarding the token there. The `(URI, String token)` constructor takes an explicit token instead, bypassing this variable. The set of three `JENESIS_REPOSITORY_*` variables mirrors the `MAVEN_REPOSITORY_*` set above. |
...
is applied first, and a source change re-runs the tests whatever the tags say. `TestModule.force(boolean)`
(defaulting to `-Djenesis.test.force`) drops the comparison altogether and executes: coverage answers whether the
tests *need* to run, and forcing is how a flaky test or a debugging session says to run them anyway.
...
`MavenRepository.filter(Predicate<String>)` wraps a repository so both `fetch` and `fetchMetadata` only
delegate when the predicate accepts the group id and report a miss otherwise - the building block for routing
group-id subsets to different repositories in a `prepend` chain.

`JenesisRepository` is the analogous refinement for the Jenesis module coordinate grammar: it extends
`Repository` with a structured `fetch(executor, module, classifier, version, type)`, and its default
coordinate-level `fetch` parses `<module>[-<classifier>][/<version>][:<type>]` (type defaulting to `jar`, with
the `jmod`-to-`jar` fallback) before delegating, so implementations only deal in parsed parts.
`JenesisRepository.of(Repository)` adapts any plain `Repository` by serialising the parts back into that
coordinate string, and `cached`/`prepend` are refined covariantly like on `MavenRepository`. Its nested
`Scope` enum specifies the detail a repository serves modules at: `Scope.MODULE` restricts it to named Java
modules, `Scope.ARTIFACT` also admits automatic modules and adjacent artifacts such as POMs.
`JenesisRepository.filter(Predicate<String>)` is the module-id counterpart of the Maven group-id filter: it
only delegates when the predicate accepts the parsed module id (classifier and version already stripped).
`JenesisRawGitRepository.groups(Predicate<String>)` returns a copy of that repository that only serves
modules whose **resolved Maven group id** the predicate accepts. Implementations: `JenesisModuleRepository`
and `JenesisRawGitRepository`.
...
the static factory `MavenDefaultRepository.of()` reads that configuration and accepts a comma-separated URL
list, chaining one repository per entry via `prepend` so entries are queried left to right; an entry may
chain `|`-separated group ids after its URL, wrapping it in `MavenRepository.filter` so it only serves
matching group ids - exact or below a dot boundary - and everything else falls through to the next entry;
an `@` entry splices in the default configuration at that position and `@<name>` the value of the system
property or environment variable `<name>`, parsed with the same grammar, so `https://nexus.corp/,@` extends
the default chain instead of replacing it - unresolved or circular references fail),
...
The static factory `JenesisModuleRepository.of(JenesisRepository.Scope)` builds the configured instance and
defaults to the public overlay at `https://repo.jenesis.build/`, appending `module/` (for `Scope.MODULE`,
restricting consumers to named Java modules) or `artifact/` (for `Scope.ARTIFACT`, allowing automatic
...
whatever that variable is set to), which also accepts a comma-separated list of base URLs: the factory chains
one repository per entry via `prepend`, so entries are queried left to right and the first hit wins. An entry
may chain `|`-separated module ids after its URL, wrapping it in `JenesisRepository.filter` so it only
serves matching module ids - exact or below a dot boundary, with classifier and version stripped before the
match - and everything else falls through to the next entry; `@` and `@<name>` reference entries splice in
the default configuration or a named property/environment value, exactly as for the Maven repository list. `JENESIS_REPOSITORY_TOKEN` is sent verbatim as an `Authorization` header on
...
back cleanly. `JenesisModuleRepository.of(...)` puts `ofLocal()` in front of whatever overlay it resolves, mirroring how
`MavenDefaultRepository.of()` consults `~/.m2/repository` before reaching for Central: a local hit
short-circuits the network fetch and a miss falls back to the overlay transparently, with no explicit
`prepend` at the call site. The `MODULAR` layout therefore wires the `module` prefix as plain
`JenesisModuleRepository.of(JenesisRepository.Scope.MODULE)` (the public overlay's `module/` root, wrapped
with `.cached(.jenesis/artifacts/)`). The `MODULAR_TO_MAVEN` layout uses `JenesisModuleRepository.of(JenesisRepository.Scope.ARTIFACT)` (the public
...
overlay because `JenesisModuleRepository.of(...)` reads it first; remove or empty that entry and the
overlay's mapping wins again.
...
`JENESIS_REPOSITORY_LOCAL` environment variable); the static factory
`JenesisModuleRepository.of(JenesisRepository.Scope)` reads that same local repository first and falls back
to the public overlay at `https://repo.jenesis.build/`, appending `module/` (strict named modules) or
`artifact/` (also serves automatic modules and adjacent artifacts like POMs) on top of the base URL (see *The public overlay* at
...
`JenesisRepository.Scope` passed to the `JenesisModuleRepository.of(...)` factory selects between them:
`Scope.MODULE` (strict, used by `Layout.MODULAR`) appends `module/`; `Scope.ARTIFACT` (permissive, used by
`Layout.MODULAR_TO_MAVEN`'s `MavenModuleResolver` for `.pom` lookups) appends `artifact/`. It is a thin Cloudflare Worker that looks
...
- **`ModularJarResolver`** is a `Resolver` (the generic resolution interface) backed by parsing each fetched
  ...
  wins by default, see `-Djenesis.resolver.module` below); the inline `<module>/<version>` spelling of a
  root coordinate; the `ModuleDescriptor.rawVersion()` recorded in the fetched jar's `module-info.class`; or omitted
  ...
  visited module emits its transitive `requires` into the queue sorted by module name. Which of several
  recorded versions survives is the one decision that is pluggable: a `ModuleVersionNegotiator` is asked for
  every `rawCompiledVersion` read, and the default `first()` keeps the earliest - so, together with the `resolved`
  set that short-circuits the next pop for an already visited module, a *later* requirer that bakes a different
  version into its own `requires <X>` directive is silently ignored. `-Djenesis.resolver.module=fail`
  swaps in the negotiator that reports that disagreement instead of discarding it, and `=ignore` the one that
  never keeps a compiled version at all; the unsafe-version check runs ahead of the negotiator under every
  mode, and a module whose version is pinned never consults it. One practical consequence: re-ordering the lines of
...
- **`MavenRepository`** extends the generic `Repository` with a structured
  ...
  default upstream URL and accepts a comma-separated list queried left to right, where a `<url>|<group>|...`
  entry only serves matching group ids (exact or below a dot boundary);
  `MAVEN_REPOSITORY_LOCAL` overrides the local repository directory; and
...
- **`MavenVersionNegotiator` / `MavenDefaultVersionNegotiator`** handle Maven's version-range syntax
  ...
  described in the Maven version comparison spec. The interface has two halves: a per-declaration `resolve`
  that turns one requested version into a concrete one, and a conflict `resolve` that settles a coordinate
  requested at several versions, defaulting to the version the traversal already chose. `MavenDefaultVersionNegotiator`
  implements the first half and offers one static factory per strategy, each overriding one or both halves -
  `maven()` (the resolver's own default), `closest()`, `latest()` and `release()`, selectable as
  `-Djenesis.resolver.maven` and described in full in the *Configuration* table.
...
- **`MavenModuleResolver`** is a `Resolver` that fetches each input Java module's POM as `<name>[/<version>]:pom`
  from a configured `Repository` (typically `JenesisModuleRepository.of(JenesisRepository.Scope.ARTIFACT)`), then hands the bytes to
...
`jenesis.project.version` so JReleaser and the emitted POM agree on the coordinate - or left to the `release`
goal below, which emits it.

### Driving a release tool from the build: the `release` goal

`release` is a top-level goal registered on every layout, and it is a **container for release tools** rather than
one tool's step. JReleaser is the only tool it currently supports; a second one would slot in beside it as
`release/<tool>`, gated the same way and sharing the goal, which is why the goal names the intent rather than the
backend and why each tool keeps its own `jenesis.<tool>.*` properties.

The goal itself is always registered, so `release` is a selector that exists whether or not anything is configured,
and a project without a release tool simply resolves it to nothing rather than reporting an unknown selector. The
**tool** is what the configuration gates: `release/jreleaser` is added only when a `jreleaser.yml`, `jreleaser.yaml`,
`jreleaser.toml` or `jreleaser.json` sits at the **project root**. JReleaser is a project-level tool that resolves
every path in its configuration against one basedir, so unlike the per-module build-configuration files this one is
looked up at the root and nowhere else; `-Djenesis.jreleaser.config=<path>` overrides that, and names a file that
must exist rather than being ignored when it does not.

    java build/jenesis/Project.java release

`release` depends on `stage`, exactly as `export` does, so the tree a release tool uploads is always the one this
build just produced. Nothing reaches it from `build`, `stage` or `export`: releasing is outward-facing and hard to
reverse, so the graph only visits it when you name it.

The JReleaser tool contributes two steps, both private to it:

- `release/jreleaser/environment` writes a `jreleaser.properties` holding `JRELEASER_PROJECT_VERSION`, the version
  this build stamped. It runs no process and has no side effect, so it is the step a CI job can run before handing
  off to a release action. Point a configuration at it once, with
  `environment: { variables: target/release/jreleaser/environment/output/jreleaser.properties }`, and the version
  travels from the build to JReleaser instead of being passed twice and drifting. Two things to know: JReleaser
  resolves `project.version` in three tiers - the value written in the configuration first, then
  `-Djreleaser.project.version`, then this file, then a `JRELEASER_*` environment variable - and `checkProperty`
  returns a configured value before it ever consults the environment. So a configuration that writes its own
  `project.version` ignores the emitted file entirely, and the file is worth wiring up precisely for the common case
  where it does not, because the version comes from CI. Where the file does apply it outranks the environment
  variable, so remove any `JRELEASER_PROJECT_VERSION` the pipeline used to export rather than leaving two channels
  to disagree. The file can only carry scalars whose fields the JReleaser reference marks as environment-resolvable,
  and it cannot
  describe artifacts, distributions or deployers, because JReleaser has no mechanism for composing one configuration
  out of several. What the build contributes structurally is the staged tree itself, which
  `deploy.maven.*.stagingRepositories` walks directly.
- `release/jreleaser/execute` forks the `jreleaser` command found on `PATH` and passes it `--basedir` (the project
  root), `--config-file` (always explicit, so a `jreleaser.yaml` is found even though JReleaser's own discovery
  probes only `jreleaser.yml`), and `--output-directory` (the step's own folder, so `trace.log` lands under
  `target/` rather than in an `out/` folder in the working tree). The process environment is forwarded unchanged, so
  every `JRELEASER_*` credential is read by JReleaser itself and never touched, logged or stored by the build.

Unlike every other tool the build drives, JReleaser is **expected from the environment rather than resolved**. The
compilers, linters, formatters and test engines Jenesis runs are fetched as pinned coordinates and recorded in the
step hash, because they *shape the artifact*: a different `kotlinc` emits different byte code, a different Checkstyle
reaches a different verdict, so reproducing a build means reproducing those versions exactly. A release tool shapes
nothing. It takes the finished, already-reproducible staged tree and transmits it, so its version changes how the
upload happens and never what was uploaded, and pinning it would buy no reproducibility while obliging the build to
track someone else's release tooling. What it does need is credentials, network access and a git identity, none of
which a dependency closure can supply. It belongs to the release environment, alongside the JDK and `git`, and is
located the way the other non-JDK tool is - `native-image` - through `GRAALVM_HOME`, then `java.home/bin`, then
`PATH`. JReleaser distributes a `jreleaser-tool-provider` artifact that would make it resolvable, and it is
deliberately not used: it is a 50 MB shaded bundle whose static initialisers mutate JVM-global state, which is a
poor trade for reproducing something that was never part of the build.

That framing is also the reason for the recommendation below. Environmental setup is what CI platforms are good at,
and a dedicated release step installs a pinned tool version and wires the secret store far better than a build tool
reaching out to `PATH` ever will.

It is a **dry run by default**. `--dry-run` performs every local phase and skips every remote one; a real release
needs `-Djenesis.jreleaser.dryRun=false`, the single switch that separates a rehearsal from a publication. That
default is not timidity: a wildcard selector schedules every top-level registration, so no combination of selectors
alone can publish. Both steps always run when named, like `export`, because their effect is outward-facing and
unchanged inputs are no evidence it already happened - with the same caveat, that a configured shared build cache
could serve such a step from an earlier identical run.

**Prefer a dedicated release integration wherever your CI offers one.** JReleaser ships a
[GitHub Action](https://github.com/jreleaser/release-action), and other platforms and ecosystems offer equivalents;
they pin the tool version, wire the platform's own secret store, and publish the release logs as build artifacts,
none of which is improved by routing it through a build tool. A release driven by CI should use those. This goal is
for what they do not cover: rehearsing a release locally before tagging, releasing from a machine or pipeline with
no such integration, and keeping the release reachable through the same selector vocabulary as the rest of the
build. Jenesis itself keeps using `jreleaser/release-action` for exactly this reason.
