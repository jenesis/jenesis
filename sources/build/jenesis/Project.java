...
public record Project(
    ...
    private static void printConfiguration() {
        String catalogue = """
                ...
                aot.enabled|false|Run the build in a JVM that loads the engine from an ahead-of-time cache, trained on the first build that finds none and reused by every later one; it applies where the engine is already compiled, as the installed CLI runs it, and not in source mode, where the JVM that compiles Make.java cannot be handed a cache; refused beside make.daemon, which keeps that engine in a JVM instead, and beside make.compile=false, which leaves nothing compiled to cache
                aot.file|.jenesis/engine.aot|Where that cache lives, relative to the project root; the engine and the JVM it is trained for are hashed into the name, as engine-<hex>.aot, so one that fits neither is simply not there and a build trains it again, sweeping the ones that no longer fit
