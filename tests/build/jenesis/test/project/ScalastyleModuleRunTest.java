...
public class ScalastyleModuleRunTest {
    ...
    public void downloads_the_pinned_scalastyle_and_writes_a_report() throws IOException {
        ...
        executor.addModule(
                "scalastyle",
                new ScalastyleModule(Map.of("maven", MavenDefaultRepository.ofEnvironment(Environment.SYSTEM)), Map.of("maven", MavenPomResolver.ofEnvironment(Environment.SYSTEM))).pinning(Pinning.STRICT),
                "project");
        executor.execute();

        Path resolved = root.resolve("scalastyle").resolve("dependencies").resolve("resolve").resolve("output").resolve("resolved");
