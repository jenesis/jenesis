...
public class DockerTest {
    ...
    @Test
    public void writes_the_configured_startup_sequence() throws IOException {
        writePlainJar(Files.createDirectory(input.resolve(BuildStep.ARTIFACTS)).resolve("app.jar"));
        SequencedProperties launcher = new SequencedProperties();
        launcher.setProperty("mainClass", "sample.Sample");
        launcher.store(input.resolve("launcher.properties"));
        SequencedProperties configuration = new SequencedProperties();
        configuration.setProperty("from", "example:24");
        configuration.setProperty("workdir", "/srv");
        configuration.setProperty("user", "sample");
        configuration.setProperty("env", "LANG=C.UTF-8\nTZ=UTC");
        configuration.setProperty("expose", "8080\n9090");
        configuration.setProperty("options", "-Xmx256m");
        configuration.setProperty("arguments", "--serve");
        Path properties = root.resolve("docker.properties");
        configuration.store(properties);

        BuildStepResult result = Docker.configured(properties).apply(
                Runnable::run,
                new BuildStepContext(previous, next, supplement),
                new LinkedHashMap<>(Map.of("input", new BuildStepArgument(
                        input,
        ...
        assertThat(result.next()).isTrue();
        assertThat(dockerfile(next.resolve(Docker.DOCKER))).containsExactly(
                "FROM example:24",
                "ENV LANG=C.UTF-8",
                "ENV TZ=UTC",
                "WORKDIR /srv",
                "COPY classpath/ /srv/classpath/",
                "EXPOSE 8080",
                "EXPOSE 9090",
                "USER sample",
                "ENTRYPOINT [\"java\", \"-Xmx256m\", \"--class-path\", \"/srv/classpath/*\", \"sample.Sample\"]",
                "CMD [\"--serve\"]");
    }

    @Test
    public void defaults_the_base_image_to_the_running_java_release() throws IOException {
        Path properties = root.resolve("docker.properties");
        new SequencedProperties().store(properties);

        assertThat(Docker.configured(properties)).isNotNull();
        writePlainJar(Files.createDirectory(input.resolve(BuildStep.ARTIFACTS)).resolve("app.jar"));
        SequencedProperties launcher = new SequencedProperties();
        launcher.setProperty("mainClass", "sample.Sample");
        launcher.store(input.resolve("launcher.properties"));

        Docker.configured(properties).apply(
                Runnable::run,
                new BuildStepContext(previous, next, supplement),
                new LinkedHashMap<>(Map.of("input", new BuildStepArgument(
                        input,
        ...
        assertThat(dockerfile(next.resolve(Docker.DOCKER)).getFirst())
                .isEqualTo("FROM eclipse-temurin:" + Runtime.version().feature() + "-jre");
    }

    @Test
    public void is_not_configured_without_a_properties_file() throws IOException {
        assertThat(Docker.configured(null)).isNull();
    }
