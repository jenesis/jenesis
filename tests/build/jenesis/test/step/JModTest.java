...
public class JModTest {
    ...
    @Test
    public void carries_the_legal_notices_it_is_handed() throws IOException {
        Path sources = Files.createDirectory(root.resolve("sources"));
        Files.writeString(sources.resolve("module-info.java"), "module sample { }\n");
        Path classes = Files.createDirectory(bundle.resolve(BuildStep.CLASSES));
        assertThat(ToolProvider.findFirst("javac").orElseThrow().run(System.out, System.err,
        ...
        Files.writeString(legal.resolve("LICENSE.txt"), "dependency licence");
        ...
        arguments.put("legal", new BuildStepArgument(root.resolve("legal"), Map.of(Path.of("legal/LICENSE"), Checksum.of(ChecksumStatus.ADDED))));
        ...
        try (ZipFile jmod = new ZipFile(next.resolve(JMod.JMODS + "sample.jmod").toFile())) {
            assertThat(new String(jmod.getInputStream(jmod.getEntry("legal/LICENSE")).readAllBytes())).isEqualTo("own licence");
