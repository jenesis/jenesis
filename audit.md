...
# Checks at the wrong chokepoint (duplication & regression-surface analysis)

A scan for validations/invariants enforced at the **wrong layer** — the same check copy-pasted across many call sites (so it can be forgotten at a new one) or missing at some sites because it is not centralized where it *cannot be bypassed*. This is the mechanism behind several bugs above: `requireSafeSegment` exists in three repositories but is absent from the staging/export steps (SEC-1/SEC-2), and `Javac`'s `shouldRun` filter is hand-maintained (INC-1…INC-11). The goal here is to name, for each check, the single structural chokepoint (a value type's constructor, a factory, a gateway method, a base class) where it should live so every present **and future** caller inherits it — preventing regressions and making the code clearer.

Thirty-four raw observations consolidated into the themes below, most-impactful first. Each notes whether the check is centralized (good), duplicated, or missing-at-some-sites, and the ideal chokepoint.

## New concrete bugs surfaced by this scan (verified)

The scan found three issues that are not just refactors but **live defects**, which should also be fixed directly:

- **CHK-BUG-1 [High] Zip-slip in `PiTestModule`.** `project/PiTestModule.java:220-236` (`extract`) resolves jar-entry names with `code.resolve(entryName)` and `Files.copy` with **no** `normalize()`/`startsWith(base)` guard — unlike the two cache extractors, which have it. **Verified.** It filters to `*.class` entries outside `META-INF/`, but an entry named `../../../home/user/.bashrc.class` passes that filter and writes outside the target. The entries come from *resolved dependency jars* (attacker-influenceable). This is a genuine traversal write the earlier security sweep missed because it only audited the cache paths. **Fix:** route through the shared containment helper (CHK-2).
- **CHK-BUG-2 [Medium] `OsvDownload` has no URI-scheme guard.** `step/OsvDownload.java:279,293` open connections via `uri.toURL().openConnection()` with no https/insecure-scheme check, and the endpoint is caller-overridable (`endpoint(URI)`). **Verified.** A misconfigured or injected `http://` OSV endpoint sends the project's dependency inventory in cleartext with no error — the exact "guard present in `Repository.open`, missing here" shape. **Fix:** route OSV requests through the network gateway (CHK-3).
- **CHK-BUG-3 [Medium] `MavenDefaultRepository` download is non-atomic across filesystems.** `maven/MavenDefaultRepository.java:362` creates the temp file in the **system** temp dir, then `materialize` (`:553`) does `Files.move(temporary, path)` into `~/.m2` **without** `ATOMIC_MOVE`. **Verified.** When `/tmp` is a different filesystem (tmpfs/containers), that move is a copy-then-delete, so a concurrent build reading `~/.m2` can observe a partial jar (which then passes the `Files.exists` fast path on later builds). **Fix:** stage in `target.getParent()` and `ATOMIC_MOVE` via the shared helper (CHK-6).

## Chokepoint themes

### CHK-1 — [High] Coordinate segment/version validation is not owned by the coordinate value type
`requireSafeSegment` (reject `/`, `\`, `..`, control chars) exists in **three** near-identical copies — `Jpx.java:563-586`, `module/JenesisModuleRepository.java:173-196`, `module/JenesisRawGitRepository.java:231-254` — and they have **already diverged** (Jpx's dot-split variant additionally rejects empty/`..` dot-segments; the module copies don't). Meanwhile the check is **missing** on: the write side (`ModularStaging`, `JenesisModuleRepositoryExport` — SEC-4 / #5, #15), TSV-derived coordinates in `JenesisRawGitRepository.Coordinate` (SEC-3 / #2), and — critically — the **Maven version**: `MavenDependencyKey`'s compact constructor validates groupId/artifactId/type/classifier but **not** version, which is validated only on the string-parse path (`Versioned`), so a version arriving via POM property interpolation (`MavenDependencyValue`) or metadata negotiation is never checked yet is spliced into `/`-delimited coordinates and filesystem/URL paths (#1, High). **Ideal chokepoint:** one shared strict `requireSafeSegment` plus validated value types — validate version inside `MavenDependencyKey.coordinate()` and a `MavenDependencyValue` compact constructor; give `JenesisRawGitRepository.Coordinate` and a `ModuleCoordinate` record validating constructors. Then no code path (present or future) can construct or emit an unvalidated coordinate. **Prevents:** the SEC-1/2/3 traversal class and coordinate-confusion at the root.

### CHK-2 — [High] Path-containment guard is copy-pasted, not a single `resolveContained`
The zip-slip / traversal guard (`base.resolve(x).normalize().startsWith(base)`) appears in `BuildExecutorFileCache.unzip`, `BuildExecutorHttpCache.unzip`, `MavenDefaultRepository` (`:253`, local-cache escape), and `JenesisModuleRepository` (`:152-157`, relativize form) — four phrasings of one idea — and is **absent** from `PiTestModule.extract` (CHK-BUG-1), `JenesisRawGitRepository` (SEC-3), and the module staging/export writers (SEC-4). **Ideal chokepoint:** one `resolveContained(base, relative)` helper (natural home beside `BuildStep.linkOrCopy` or on `PathPlacement`) that is the *only* way any code turns external/derived text into a path under a root, plus a shared `Archives.extract(source, target, predicate)` that every jar/zip explode goes through. **Prevents:** the next extractor or path-builder silently reintroducing traversal — "forgot the check" becomes impossible.

### CHK-3 — [High] The outbound-HTTP gateway is bypassed by some callers
`Repository.open` is the intended single gateway (scheme allowlist, hand-rolled redirects, same-origin credential scoping, retry/backoff) — but it is **missing timeouts** (IO-1) and is **bypassed** by `OsvDownload` (CHK-BUG-2: no scheme check, default redirect following, no retry) and partially re-implemented by `BuildExecutorHttpCache` (a second, divergent scheme check, `:181-189`). **Ideal chokepoint:** funnel every outbound request — artifact, cache, and OSV (extend `open` to carry a POST body/headers) — through one `Repository.open`/`HttpGateway`, so scheme policy, timeouts, redirect and credential rules apply once. **Prevents:** the next network step copying the raw `HttpURLConnection` pattern and shipping without the plaintext refusal or timeout.

### CHK-4 — [High, structural] Nothing enforces the `shouldRun`-declares-what-`apply`-reads contract
This is the structural root of the entire INC-1…INC-11 stale-output class. `BuildStep.shouldRun` is a free-form hand-written filter (`BuildStep.java:39`, enforced only at `BuildExecutorDefault.java:143`); nothing checks that the prefixes it declares match the files `apply` actually reads. Every override that reads a new file must be manually kept in sync — and repeatedly wasn't (Inventory/Sbom/Versions/Javac). **Ideal chokepoint:** replace the hand-written `shouldRun` with a declared read-set on `BuildStep` — e.g. `default Collection<Path> trackedPrefixes()` (or a `TrackedBuildStep` base) — and derive `shouldRun` from it *finally*, so a step declares its inputs in one place and change-detection is generated, not transcribed. Ideally the same declaration drives what `apply` is *allowed* to read (fail-fast in tests if it reads outside its declared set). **Prevents:** the whole "added a read, forgot the filter → stale artifact" regression class going forward — directly the user's stated goal.

### CHK-5 — [Medium] Checksum grammar + verification is hand-rolled at ~5 sites, one crash-prone
Parsing `<algorithm>/<hex>` and stream-compare is re-implemented in `Resolver.validate` (`:74-92` — `indexOf('/')` unchecked, so a pin lacking `/` throws a bare `StringIndexOutOfBoundsException` with no coordinate/file context), `MavenPomResolver.drainAndValidate` (`:687-734`, the only copy with a helpful error), `MavenDefaultRepository` (inline `FileChannel` digest + hex parse, twice), and `Jpx.Installation.verify`. Strict-pin enforcement is separately duplicated (`Dependencies.java:270` for BOMs vs `:451` for artifacts, already divergent) and **jpx bypasses it entirely**. **Ideal chokepoint:** a `Checksum`/`ChecksumSpec` value type whose constructor parses+validates the grammar (known algorithm, even-length hex, min length) with a `verify(Path|InputStream, context)` implemented once; and enforce strict-pinning inside a single `Resolver.materialize(…, Pinning)` where every artifact and BOM becomes a file. **Prevents:** parsers drifting (a format one path accepts and another rejects silently weakens verification), the SIOOBE-on-typo footgun, and new fetch paths shipping without strict-pin enforcement.

### CHK-6 — [Medium] Atomic-publish (temp-in-target + `ATOMIC_MOVE`) is copied 5+ times, one wrong
The correct pattern (temp in `target.getParent()`, `ATOMIC_MOVE`, cleanup on failure) is independently implemented in `Repository.cached`, `Jpx` (spill + descriptor), and both cache backends — and implemented **wrong** in `MavenDefaultRepository` (CHK-BUG-3: system-temp dir → cross-filesystem non-atomic move). `DownloadModuleUris` (IO-5) and `Sbom`/`Bom` write in place with no atomicity at all. **Ideal chokepoint:** one `atomicWrite(target, writer)` helper beside `BuildStep.linkOrCopy` that guarantees same-filesystem staging + `ATOMIC_MOVE` (with the `AtomicMoveNotSupported` fallback) and temp cleanup. **Prevents:** torn files in shared `~/.m2`/`~/.jenesis` observed by concurrent readers, and further per-site drift.

### CHK-7 — [Medium] Coordinate→filename mapping has a correct chokepoint that two sites bypass
`BuildExecutorModule.encode` (URL-encoding, injective) is the correct coordinate→filename mapping (used by `Repository.cached`), but `Jpx.java:405` and `Dependencies.java:423` instead use a **lossy** `coordinate.replace('/','-') + ".jar"`, and `Dependencies` then silently unifies collisions (`:426-437`). **Ideal chokepoint:** a `BuildExecutorModule.fileName(coordinate)` beside `encode` as the single mapping for every flat folder. **Prevents:** two distinct coordinates colliding to one jar (wrong artifact on the classpath) or a misleading "Conflicting checksums pinned" abort.

### CHK-8 — [Medium] Three copies of repository-chain parsing, already drifting
The `@name` indirection + cycle detection + `|group` filter + default-URI + trailing-slash logic (~80 lines) is copied in `MavenDefaultRepository.chain`, `JenesisModuleRepository.chain`, and `JenesisRawGitRepository.chain` — and the third has **already diverged** (it threads an `inherited` predicate the others lack). **Ideal chokepoint:** one generic `Repository.parseChain(text, defaultUri, envVar, leafFactory, prepend)`. **Prevents:** a security/correctness fix to `@`-resolution or cycle bookkeeping landing in one copy but not the others.

### CHK-9 — [Medium] Two byte-identical cache-key `fold` copies feed one layered cache
`BuildExecutorFileCache.fold` and `BuildExecutorHttpCache.fold` are byte-for-byte copies, and `BuildExecutorLayeredCache` composes both **assuming their independently-computed keys agree**. (Both also share the filesystem-order non-determinism of INC-B1.) **Ideal chokepoint:** a single `BuildExecutorCache.fold(…)` / `CacheKey` computed once by `BuildExecutorDefault` and passed to both layers. **Prevents:** a tweak to one copy silently making the local and remote layers address disjoint key spaces (remote cache never warms).

### CHK-10 — [Medium] Secure-XML factory duplicated inline (currently safe, regression-prone)
`MavenDefaultVersionNegotiator.toDocumentBuilderFactory` builds a hardened `DocumentBuilderFactory` (secure-processing + `disallow-doctype-decl`), but `MavenRepositoryExport` and `MavenRepositoryStaging` construct it **inline** instead. **Verified those inline copies currently DO set the guard — so this is not an active XXE hole**, but it is a duplicated security-critical construction: the next POM/metadata parse site that copies the pattern and omits one line reintroduces XXE against attacker-influenced XML. **Ideal chokepoint:** promote it to a public `Xml.secureDocumentBuilderFactory()` / `Xml.parse(stream)` as the only way build code parses XML.

### Lower-severity consolidations (same pattern, smaller blast radius)
- **CHK-11 [Med] Pin-value / index-entry micro-formats** (`<version|:classifier> [checksum]` and `<path> [checksum]`) are hand-split at ~11 sites; the `:`-classifier guard is applied at only some (`MavenPomResolver.java:41-43` lacks it), so a `:classifier` pin is treated as a literal version there → silent mis-resolution (#11/#17). → `PinValue`/`IndexEntry` value types with a `parse` factory.
- **CHK-12 [Med] Version-guard on jar module descriptors** diverged *within one file* — `ModularJarResolver.java:172` checks only the leading `:`, while `:205-211` does the full `..`/`/`/`\` check (#18). → one `requireSafeVersion`.
- **CHK-13 [Med] `RepositoryItem` eager-open** (IO-3/BUG-10): laziness is a per-repository discipline that `JenesisModuleRepository`/`JenesisRawGitRepository` get wrong. → a `RepositoryItem.ofUri(uri, token, retry)` factory that makes laziness a property of the value type.
- **CHK-14 [Low] `jpx` command validation is mid-`install`, not at construction** (#10): a `Command` with an unsafe name/version can exist and derive paths before any check → validate in `Command.parse`/constructor. Related: the modulepath/classpath entries read back from `jpx.properties` are resolved unvalidated (#30) → validate in a single `Installation.entry(name)`.
- **CHK-15 [Low] File-digest loop + checksum-text parsing** re-inlined in `MavenDefaultRepository` instead of using `HashDigestFunction` (#33); **local-repo location resolution** diverged read-vs-write side (#32); the **tool-requirements `Requires` step** is copied in ~14 tool modules (#34) → one `ToolRequirements` step. Each is a maintainability/drift risk, not a live bug.

## Why this matters (the user's goal)

The recurring shape — a check implemented well in one place, copied imperfectly to a second, and forgotten at a third — is precisely how SEC-1/SEC-2 (missing `requireSafeSegment`), CHK-BUG-1 (missing zip-slip guard), and the INC-* stale-output bugs arose. Moving each check to a chokepoint it *cannot be bypassed at* (a validating constructor, a single gateway, a generated `shouldRun`) converts "remember to add the guard" into "the guard is structurally unavoidable," which both prevents the next regression and removes the duplicated code. The highest-leverage moves are **CHK-4** (declared read-sets — kills the incremental-staleness class), **CHK-1/CHK-2** (coordinate value types + `resolveContained` — kills the traversal class), and **CHK-3/CHK-5/CHK-6** (network/integrity/atomic-write gateways).

---

# Convergence re-audit (Fable, iterated)

A follow-up pass re-ran all seven audit dimensions on Fable agents over four rounds, feeding every already-documented finding forward each round so agents would surface only *new* issues, and looping until a round produced no new Critical/High/Medium.

**It did not converge.** New-major counts per round were **37 → 19 → 20 → 12** (161 findings total; round 4's `incremental` and `testgaps` agents aborted on a session limit). The failure to converge is itself a result: with nothing being fixed between rounds, a converging process would trend to zero, so the persistent ~12–20 "majors" per round is largely Fable's **false-positive / re-phrasing tail** — the same issues restated, plus speculative findings that don't survive tracing. The signal is therefore the *verified* subset below, not the raw 161 (which are archived in the run journal). This is an honest limitation of "loop until only minor findings" with a fast model: the model's finding stream doesn't dry up, so a human/verified filter is the real stopping condition.

## Verified new findings (traced against the source)

### CV-1 — [High] A single repository token is attached to *every* repository in a chained configuration (credential leak to public mirrors)
`module/JenesisModuleRepository.java:78-83` (and `module/JenesisRawGitRepository.java:141`). **Verified.** `chain()` threads one token (`jenesis.module.token`/`JENESIS_REPOSITORY_TOKEN`) into the constructor of *every* leaf repository in the comma-separated chain. If a user configures a private repository (needing the token) alongside a public mirror/fallback in the same chain, the private token is attached to the public repository too, and `Repository.open`'s `sameOrigin` check *permits* sending it to that mirror's own host. A private credential is thus transmitted to a public server. **Fix:** scope the token per-entry (only the entry it was configured for), e.g. carry the token on the specific URI it belongs to rather than the whole chain.

### CV-2 — [Medium-High] A remote redirect can downgrade to `file://` and read local files as artifact content
`Repository.java:136-143`. **Verified.** The scheme allowlist permits both `https` and `file` (the latter for legitimate local repositories), and redirects are followed by re-entering the loop with `current = current.resolve(location)`. So an `https` origin that responds `Location: file:///etc/passwd` passes the scheme check on the next iteration and the non-`HttpURLConnection` branch returns `connection.getInputStream()` — reading a local file and handing it back as the fetched artifact/POM/metadata. **Fix:** once a fetch has started over `https`, refuse a redirect that changes the scheme (in particular to `file`); only allow `file` when it is the *originally configured* repository URI.

### CV-3 — [Medium] XML injection into the synthesized alias POM via the unescaped negotiated version — ✅ FIXED
**Fixed** in `maven/MavenAliasResolver.pom()`: the synthetic POM is now assembled through the DOM (`createElementNS` + `setTextContent`, serialized via `Transformer`, matching the other emitters), so every interpolated value — including the RELEASE-negotiated version — is escaped and cannot break out of its element. Regression test: `MavenAliasResolverTest.escapes_negotiated_version_preventing_pom_injection`.

`maven/MavenAliasResolver.java:190-205` (original). **Verified** (a defect in the module-alias feature added this session). `pom()` builds the synthetic discovery POM with `String.formatted`, interpolating the version with no XML escaping; on the unpinned alias path that version is the `<release>` text of the target's `maven-metadata.xml` fetched from the remote repository (`:89`, RELEASE negotiation), which is never validated for XML metacharacters. A malicious/compromised repository serving a `<release>` value like `1.0</version></dependency><dependency>…evil…<version>1.0` yields a well-formed synthetic POM carrying an attacker-chosen extra `<dependency>`, which `MavenPomResolver` then resolves onto the classpath. The sibling emitters build XML safely via DOM `setTextContent`; this path is the outlier. **Fix:** build the POM with the DOM API (or XML-escape every interpolated value), and validate the negotiated version string. *(Small, localized — worth fixing directly since it was introduced here.)*

### CV-4 — [Medium] Imported-BOM precedence is decided by `HashMap` iteration order, not declaration order
`maven/MavenPomResolver.java:511` (`managedDependencies = new HashMap<>()`) consumed by `resolve()`/`flattenImport()` via `putIfAbsent`. When a POM imports two BOMs managing the same coordinate at different versions, the winner is whichever import hashes first rather than the first-declared (Maven's rule) — and can silently flip when an unrelated entry changes. The `dependencies` map one line later uses `LinkedHashMap`, showing order was intended to matter. **Fix:** use a `LinkedHashMap` for `managedDependencies` and preserve document order when merging.

### CV-5 — [Medium] An unfetchable POM silently becomes an empty stub, bypassing its declared checksum and dropping parent/BOM management
`maven/MavenPomResolver.java:668-687` (`assembleOrCached`). When `repository.fetch` returns empty for a POM, the code fabricates an empty `UnresolvedPom` and the `checksum` parameter is ignored (`drainAndValidate` runs only in the `candidate != null` branch). So a checksum-pinned import BOM that fails to fetch is silently treated as managing nothing (the pin never fails — it evaporates), and a missing parent POM yields an empty parent (parent-managed versions/properties vanish) instead of hard-failing as Maven does. **Fix:** throw when `checksum != null` and the POM can't be fetched, and fail (or warn) on a missing parent; reserve the lenient stub for ordinary transitive artifact POMs.

### CV-6 — [Low] JSON injection into the OSV `querybatch` body
`step/OsvDownload.java:264-276`. The querybatch JSON is hand-concatenated from `groupId`/`artifactId`/`version`; `MavenDependencyKey.validate` blocks `/ \ . ..` but **not** `"`, and a version from remote metadata isn't re-validated. A coordinate/version containing a quote can reshape the query so its advisories aren't matched (silently evading the vuln gate for that dependency) or malform the body. Constrained (the tainted coordinate must still resolve/download to appear), hence Low. **Fix:** serialize the body with the existing `Json` emitter.

## Reported but unverified (candidates for manual follow-up)

These recurred across rounds but I did **not** confirm them (some contradict earlier verified traces; treat as leads, not findings): `Versions.java:41` may parse `requires.properties` with the wrong segment count (would mean requires-version stamping never runs — contradicts the INC-7 assumption, needs a focused trace); `BuildExecutorDefault` "crash in the commit window permanently poisons a step" (contradicts the engine trace that a missing `step.properties` forces a safe rebuild — INC-11); `InferredMultiProjectAssembler` reading `process-*.properties` outside change-tracking; `Java.java:174` emitting `--add-modules ALL-MODULE-PATH` without a module path for classpath-only runs; serial POM-closure fetching in `MavenPomResolver` (perf). Several are plausible and worth a look; they are listed here rather than asserted because the convergence run's precision was too low to trust unverified.

## Net from the iterated pass

Six verified new findings (CV-1…CV-6), of which **CV-1 (credential leak)** and **CV-2 (redirect→`file://`)** are the most important and were not caught by the earlier single-pass sweeps. CV-3 is a real injection in code added this session and should be fixed directly. The exercise also demonstrates that "re-run until only minor findings" does not terminate cleanly with a fast model — the durable finding set comes from tracing, and the highest-value structural fixes remain those in the chokepoint section (CHK-1…CHK-6).
