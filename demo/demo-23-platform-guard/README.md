...
Selects a dependency **variant**, and lets the machine decide which one. Some
artifacts publish several jars under one coordinate, told apart by a Maven
*classifier* - the same module name, different bytes - and a project often wants
a different one per operating system. Both halves live in the pin: the classifier
in its value, and a **guard** at the end of the line that says when the line
applies.
...
No `target/` cleanup is needed between the two runs: adding or dropping the flag
re-resolves exactly what it affects.
...
A classifier is part of the pin *value*, with a leading-colon qualifier
`:<classifier>[:<version>]`, so the key stays the bare module name and the choice
applies wherever the module appears in the closure - only one variant of a module
name can be selected, as the module path allows only one module of a name. The
entry point proves the classified variant was resolved: the `jdk-flow` jar
requires nothing but `java.base`, while the default one requires
`org.reactivestreams`.
