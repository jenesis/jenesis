...
Packaged as well as executed
----------------------------

An agent that only ran when the build ran it would be a build-time trick rather than a
deployment. A `packaging.properties` with `bundle=true` therefore ships this module as a
`bundle.zip`, and the launch it carries hands the agent to the JVM exactly as `Execute`
does:
...
The agent jar sits in the same flat `jars/` store as every other dependency - an agent is
an ordinary dependency that is also named to the JVM, not a second kind of artifact - and
only an agent this artifact actually ships is named, so the test module's Mockito is no
part of the main module's launch. A generated `Dockerfile` carries the same option with
`/app/jars/` paths.
